...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建数据同步任务A1，源表：数据源A下schema_A.table_B，目标表：数据源A下schema_A.table_C，其余内容均正常填写，保存并提交
2. 查看【任务依赖配置有误】弹窗
3. 点击【任务依赖配置有误】页面的“确定”按钮后，进入数据同步任务A1的【调度属性-任务间依赖】添加依赖任务：项目A下的数据同步任务G，保存后点击“提交”
4. 点击【任务依赖配置有误】页面的“确定”按钮后，进入数据同步任务A1的【调度属性-任务间依赖】添加依赖任务：项目A下的数据同步任务A，项目B下的数据同步任务A，保存后点击“提交”
5. 点击【任务依赖配置有误】页面的“确定”按钮后，进入数据同步任务A1的【调度属性-任务间依赖】移除依赖任务：项目A下的数据同步任务G，保存后点击“提交”

**预期**:
1. 进入【任务依赖配置有误】弹窗：
2. 1）title：任务依赖配置有误
3. 2）展示字段依次为：任务、租户、项目
4. 3）“任务”右侧展示问号icon，hover展示：“当前任务配置存在缺失或者多余的上游依赖任务”
5. 4）“确定、仍要提交”按钮
6. 展示任务如下：
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建数据同步任务A1，源表：数据源A下schema_A.table_B，目标表：数据源A下schema_A.table_C，其余内容均正常填写，保存并提交
2. 进入租户A下项目B的【数据开发-周期任务】页面，新建数据同步任务B1，源表：数据源A下schema_B.table_A，目标表：数据源A下schema_A.table_C，其余内容均正常填写，保存并提交
3. 进入租户A下项目B的【数据开发-周期任务】页面，新建数据同步任务C1，源表：数据源A下schema_B.table_B，目标表：数据源A下schema_A.table_C，其余内容均正常填写，保存并提交

**预期**:
1. 进入【任务依赖配置有误】弹窗：
2. 1）展示缺失上游依赖任务如下：
3. a）租户A，项目A下的数据同步任务A
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建数据同步任务D1，源表：数据源B下schema_A.table_A，目标表：数据源A下schema_A.table_C，其余内容均正常填写，保存并提交
2. 进入租户A下项目B的【数据开发-周期任务】页面，新建数据同步任务E1，源表：数据源B下schema_A.table_B，目标表：数据源A下schema_A.table_C，其余内容均正常填写，保存并提交
3. 进入租户A下项目B的【数据开发-周期任务】页面，新建数据同步任务F1，源表：数据源B下schema_B.table_A，目标表：数据源A下schema_A.table_C，其余内容均正常填写，保存并提交
4. 进入租户A下项目B的【数据开发-周期任务】页面，新建数据同步任务G1，源表：数据源B下schema_B.table_B，目标表：数据源A下schema_A.table_C，其余内容均正常填写，保存并提交
5. 进入租户A下项目B的【数据开发-周期任务】页面，新建数据同步任务H1，源表：数据源C下schema_A.table_B，目标表：数据源A下schema_A.table_C，其余内容均正常填写，保存并提交

**预期**:
1. 进入【任务依赖配置有误】弹窗：
2. 1）展示缺失上游依赖任务如下：
3. a）租户A，项目A下的数据同步任务D
4. b）租户A，项目B下的数据同步任务D
5. 2）点击“仍要提交”：进入提交弹窗，确认后，任务提交成功
6. 进入【任务依赖配置有误】弹窗：
7. 1）展示缺失上游依赖任务如下：
...
## 验证hivesql任务与数据同步间任务依赖功能正常_数据同步任务
**优先级**: P1
**前置条件**: 1）数据源信息：
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建数据同步任务A，源表：数据源A下schema_meta.table_A，其余内容均正常填写，保存并提交
2. 进入租户A下项目B的【数据开发-周期任务】页面，新建数据同步任务B，源表：数据源A下schema_B.table_A，其余内容均正常填写，保存并提交
3. 进入租户A下项目B的【数据开发-周期任务】页面，新建数据同步任务C，源表：数据源A下table_B，其余内容均正常填写，保存并提交
4. 进入租户A下项目B的【数据开发-周期任务】页面，新建数据同步任务D，源表：数据源B下schema_B.table_A，其余内容均正常填写，保存并提交
5. 进入租户A下项目B的【数据开发-周期任务】页面，新建数据同步任务E，源表：数据源C下schema_B.table_A，其余内容均正常填写，保存并提交

**预期**:
1. 进入【任务依赖配置有误】弹窗：
2. 1）展示缺失上游依赖任务如下：
3. a）租户A，项目B下的hivesql任务A
4. 2）点击“仍要提交”：进入提交弹窗，确认后，任务提交成功
5. 进入【任务依赖配置有误】弹窗：
6. 1）展示缺失上游依赖任务如下：
...
## 验证mysql任务与数据同步间任务依赖功能正常_数据同步任务
**优先级**: P2
**前置条件**: 1）数据源信息：
...
## 验证dorissql任务与数据同步间任务依赖功能正常_数据同步任务
**优先级**: P2
**前置条件**: 1）数据源信息：
...
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建数据同步任务A，源表：数据源A下schema_meta.table_A，其余内容均正常填写，保存并提交
2. 进入租户A下项目B的【数据开发-周期任务】页面，新建数据同步任务B，源表：数据源A下schema_B.table_A，其余内容均正常填写，保存并提交
3. 进入租户A下项目B的【数据开发-周期任务】页面，新建数据同步任务D，源表：数据源B下schema_B.table_A，其余内容均正常填写，保存并提交
4. 进入租户A下项目B的【数据开发-周期任务】页面，新建数据同步任务E，源表：数据源A下schema_B.table_A，其余内容均正常填写，保存并提交

**预期**:
1. 进入【任务依赖配置有误】弹窗：
2. 1）展示缺失上游依赖任务如下：
3. a）租户A，项目B下的dorissql任务A
4. 2）点击“仍要提交”：进入提交弹窗，确认后，任务提交成功
5. 进入【任务依赖配置有误】弹窗：
6. 1）展示缺失上游依赖任务如下：
...
## 验证hivesql任务间任务依赖功能正常_hivesql初始化方式创建_自动依赖
**优先级**: P2
**前置条件**: 1）项目A、项目B的hadoop引擎的初始化方式均为创建
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建hivesql任务A1，脚本如下：select * from table_A;【调度属性-任务间依赖】选择“自动依赖”，刷新
2. 修改脚本如下：select * from table_B_1;【调度属性-任务间依赖】的自动依赖，刷新
3. 修改脚本如下：select * from table_B_2;【调度属性-任务间依赖】的自动依赖，刷新
4. 修改脚本如下：select * from schema_B.table_B_2;【调度属性-任务间依赖】的自动依赖，刷新
5. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_A t1 left join schema_B.table_B_1 t2 on t1.col=t2.col) t3 left join schema_B.table_B_2 t4 on t3.col=t4.col;【调度属性-任务间依赖】的自动依赖，刷新
6. 修改脚本如下：create table table_C_1 like table_A;【调度属性-任务间依赖】的自动依赖，刷新
7. 修改脚本如下：create table table_C_2 as select * from table_A;【调度属性-任务间依赖】的自动依赖，刷新
8. 修改脚本如下：insert into schema_B.table_B_1 select * from table_A;【调度属性-任务间依赖】的自动依赖，刷新
9. 修改脚本如下：insert overwrite table table_C_2 select t1.id,t2.name,t3.age from table_A t1 left join schema_B.table_A t2 on t1.col=t2.col;【调度属性-任务间依赖】的自动依赖，刷新
10. 修改脚本如下：select * from table_A;select * from table_B_1;select * from table_B_2;select * from schema_B.table_B_2;select * from (select t1.id,t2.name,t3.age from table_A t1 left join schema_B.table_B_1 t2 on t1.col=t2.col) t3 left join schema_B.table_B_2 t4 on t3.col=t4.col;create table table_C_1 like table_A;create table table_C_2 as select * from table_A;insert into schema_B.table_B_1 select * from table_A;insert overwrite table table_C_2 select t1.id,t2.name,t3.age from table_A t1 left join schema_B.table_A t2 on t1.col=t2.col;【调度属性-任务间依赖】的自动依赖，刷新

**预期**:
1. 1）自动依赖任务如下：
2. a）租户A，项目B下的hivesql任务A
3. 2）提交任务，任务提交成功，系统给出提交成功提示，状态更新为下一处理阶段
4. 1）自动依赖任务如下：
5. a）租户A，项目B下的hivesql任务D
6. 2）提交任务，任务提交成功，系统给出提交成功提示，状态更新为下一处理阶段
7. 1）自动依赖任务如下：
8. a）租户A，项目B下的hivesql任务C、E
9. 2）提交任务，任务提交成功，系统给出提交成功提示，状态更新为下一处理阶段
10. 1）自动依赖任务如下：空
...
## 验证hivesql任务间任务依赖功能正常_hivesql初始化方式创建_手动依赖_手动添加
**优先级**: P2
**前置条件**: 1）项目A、项目B的hadoop引擎的初始化方式均为创建
2）hivesql任务（租户A下，在项目A、项目B均配置以下任务且已提交）：
a）hivesql任务A：create table table_A;
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建hivesql任务A1，脚本如下：select * from table_A;【调度属性-任务间依赖】选择“手动依赖”，保存并提交
2. 修改脚本如下：select * from table_B_1;保存并提交
3. 修改脚本如下：select * from table_B_2;保存并提交
4. 修改脚本如下：select * from schema_B.table_B_2;保存并提交
5. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_A t1 left join schema_B.table_B_1 t2 on t1.col=t2.col) t3 left join schema_B.table_B_2 t4 on t3.col=t4.col;保存并提交
6. 修改脚本如下：create table table_C_1 like table_A;保存并提交
7. 修改脚本如下：create table table_C_2 as select * from table_A;保存并提交
8. 修改脚本如下：insert into schema_B.table_B_1 select * from table_A;保存并提交
9. 修改脚本如下：insert overwrite table table_C_2 select t1.id,t2.name,t3.age from table_A t1 left join schema_B.table_A t2 on t1.col=t2.col;保存并提交
10. 修改脚本如下：select * from table_A;select * from table_B_1;select * from table_B_2;select * from schema_B.table_B_2;select * from (select t1.id,t2.name,t3.age from table_A t1 left join schema_B.table_B_1 t2 on t1.col=t2.col) t3 left join schema_B.table_B_2 t4 on t3.col=t4.col;create table table_C_1 like table_A;create table table_C_2 as select * from table_A;insert into schema_B.table_B_1 select * from table_A;insert overwrite table table_C_2 select t1.id,t2.name,t3.age from table_A t1 left join schema_B.table_A t2 on t1.col=t2.col;保存并提交
...
## 验证hivesql任务间任务依赖功能正常_hivesql初始化方式创建_手动依赖_依赖推荐
**优先级**: P2
**前置条件**: 1）项目A、项目B的hadoop引擎的初始化方式均为创建
2）hivesql任务（租户A下，在项目A、项目B均配置以下任务且已提交）：
a）hivesql任务A：create table table_A;
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建hivesql任务A1，脚本如下：select * from table_A;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
2. 修改脚本如下：select * from table_B_1;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
3. 修改脚本如下：select * from table_B_2;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
4. 修改脚本如下：select * from schema_B.table_B_2;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
5. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_A t1 left join schema_B.table_B_1 t2 on t1.col=t2.col) t3 left join schema_B.table_B_2 t4 on t3.col=t4.col;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
6. 修改脚本如下：create table table_C_1 like table_A;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
7. 修改脚本如下：create table table_C_2 as select * from table_A;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
8. 修改脚本如下：insert into schema_B.table_B_1 select * from table_A;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
9. 修改脚本如下：insert overwrite table table_C_2 select t1.id,t2.name,t3.age from table_A t1 left join schema_B.table_A t2 on t1.col=t2.col;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
10. 修改脚本如下：select * from table_A;select * from table_B_1;select * from table_B_2;select * from schema_B.table_B_2;select * from (select t1.id,t2.name,t3.age from table_A t1 left join schema_B.table_B_1 t2 on t1.col=t2.col) t3 left join schema_B.table_B_2 t4 on t3.col=t4.col;create table table_C_1 like table_A;create table table_C_2 as select * from table_A;insert into schema_B.table_B_1 select * from table_A;insert overwrite table table_C_2 select t1.id,t2.name,t3.age from table_A t1 left join schema_B.table_A t2 on t1.col=t2.col;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”

**预期**:
1. 1）依赖推荐任务如下：
2. a）租户A，项目B下的hivesql任务A
3. 2）未添加推荐依赖，保存并提交任务，进入【任务依赖配置有误】弹窗：
4. a）展示缺失上游依赖任务如下：
5. i）租户A，项目B下的hivesql任务A
6. b）点击“仍要提交”：进入提交弹窗，确认后，任务提交成功
7. 1）依赖推荐任务如下：
8. a）租户A，项目B下的hivesql任务D
...
## 验证hivesql任务间任务依赖功能正常_hivesql初始化方式不创建且不对接schema_自动依赖
**优先级**: P2
**前置条件**: 1）项目A、项目B的hadoop引擎的初始化方式均为不创建且不对接schema
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建hivesql任务A1，选择schema_C，脚本如下：select * from schema_B.table_A;【调度属性-任务间依赖】选择“自动依赖”，刷新
2. 修改脚本如下：select * from schema_B.table_B_1;【调度属性-任务间依赖】选择“自动依赖”，刷新
3. 修改脚本如下：select * from schema_B.table_B_2;【调度属性-任务间依赖】选择“自动依赖”，刷新
4. 修改脚本如下：select * from table_A;【调度属性-任务间依赖】选择“自动依赖”，刷新
5. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_A t1 left join schema_B.table_B_1 t2 on t1.col=t2.col) t3 left join schema_B.table_B_2 t4 on t3.col=t4.col;【调度属性-任务间依赖】选择“自动依赖”，刷新
6. 修改脚本如下：create table table_C_1 like table_A;【调度属性-任务间依赖】选择“自动依赖”，刷新
7. 修改脚本如下：create table table_C_2 as select * from table_A;【调度属性-任务间依赖】选择“自动依赖”，刷新
8. 修改脚本如下：insert into schema_B.table_B_1 select * from table_A;【调度属性-任务间依赖】选择“自动依赖”，刷新
9. 修改脚本如下：insert overwrite table table_C_2 select t1.id,t2.name,t3.age from table_A t1 left join schema_B.table_A t2 on t1.col=t2.col;【调度属性-任务间依赖】选择“自动依赖”，刷新
10. 修改脚本如下：select * from schema_B.table_A;select * from schema_B.table_B_1;select * from schema_B.table_B_2;select * from table_A;select * from (select t1.id,t2.name,t3.age from table_A t1 left join schema_B.table_B_1 t2 on t1.col=t2.col) t3 left join schema_B.table_B_2 t4 on t3.col=t4.col;create table table_C_1 like table_A;create table table_C_2 as select * from table_A;insert into schema_B.table_B_1 select * from table_A;insert overwrite table table_C_2 select t1.id,t2.name,t3.age from table_A t1 left join schema_B.table_A t2 on t1.col=t2.col;【调度属性-任务间依赖】选择“自动依赖”，刷新

**预期**:
1. 1）自动依赖任务如下：空
2. 2）提交任务，任务提交成功，系统给出提交成功提示，状态更新为下一处理阶段
3. 1）自动依赖任务如下：
4. a）租户A，项目A下的hivesql任务B、D
5. b）租户A，项目B下的hivesql任务B、D
6. 2）提交任务，任务提交成功，系统给出提交成功提示，状态更新为下一处理阶段
7. 1）自动依赖任务如下：
8. a）租户A，项目A下的hivesql任务C、E
9. b）租户A，项目B下的hivesql任务C、E
...
## 验证hivesql任务间任务依赖功能正常_hivesql初始化方式不创建且不对接schema_手动依赖_手动添加
**优先级**: P2
**前置条件**: 1）项目A、项目B的hadoop引擎的初始化方式均为不创建且不对接schema
2）hivesql任务（租户A下，在项目A、项目B均配置以下任务且已提交，所选schema均为schema_C）：
a）hivesql任务A：create table table_A;
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建hivesql任务A1，选择schema_C，脚本如下：select * from schema_B.table_A;【调度属性-任务间依赖】选择“手动依赖”，保存并提交
2. 修改脚本如下：select * from schema_B.table_B_1;保存并提交
3. 修改脚本如下：select * from schema_B.table_B_2;保存并提交
4. 修改脚本如下：select * from table_A;保存并提交
5. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_A t1 left join schema_B.table_B_1 t2 on t1.col=t2.col) t3 left join schema_B.table_B_2 t4 on t3.col=t4.col;保存并提交
6. 修改脚本如下：create table table_C_1 like table_A;保存并提交
7. 修改脚本如下：create table table_C_2 as select * from table_A;保存并提交
8. 修改脚本如下：insert into schema_B.table_B_1 select * from table_A;保存并提交
...
**预期**:
1. 系统提示任务提交成功，任务状态更新为【待运行/调度中/运行中】
2. 进入【任务依赖配置有误】弹窗：
3. 1）展示缺失上游依赖任务如下：
4. a）租户A，项目A下的hivesql任务B、D
5. b）租户A，项目B下的hivesql任务B、D
6. 2）点击“仍要提交”：进入提交弹窗，确认后，任务提交成功
7. 进入【任务依赖配置有误】弹窗：
8. 1）展示缺失上游依赖任务如下：
9. a）租户A，项目A下的hivesql任务C、E
10. b）租户A，项目B下的hivesql任务C、E
...
## 验证hivesql任务间任务依赖功能正常_hivesql初始化方式不创建且不对接schema_手动依赖_依赖推荐
**优先级**: P2
**前置条件**: 1）项目A、项目B的hadoop引擎的初始化方式均为不创建且不对接schema
2）hivesql任务（租户A下，在项目A、项目B均配置以下任务且已提交，所选schema均为schema_C）：
a）hivesql任务A：create table table_A;
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建hivesql任务A1，选择schema_C，脚本如下：select * from schema_B.table_A;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
2. 修改脚本如下：select * from schema_B.table_B_1;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
3. 修改脚本如下：select * from schema_B.table_B_2;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
4. 修改脚本如下：select * from table_A;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
5. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_A t1 left join schema_B.table_B_1 t2 on t1.col=t2.col) t3 left join schema_B.table_B_2 t4 on t3.col=t4.col;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
6. 修改脚本如下：create table table_C_1 like table_A;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
7. 修改脚本如下：create table table_C_2 as select * from table_A;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
8. 修改脚本如下：insert into schema_B.table_B_1 select * from table_A;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
...
**预期**:
1. 1）依赖推荐任务如下：空
2. 2）未添加推荐依赖，保存并提交任务，任务提交成功，系统给出提交成功提示，状态更新为下一处理阶段
3. 1）依赖推荐任务如下：
4. a）租户A，项目A下的hivesql任务B、D
5. b）租户A，项目B下的hivesql任务B、D
6. 2）未添加推荐依赖，保存并提交任务，进入【任务依赖配置有误】弹窗：
7. a）展示缺失上游依赖任务如下：
8. i）租户A，项目A下的hivesql任务B、D
9. ii）租户A，项目B下的hivesql任务B、D
...
## 验证hivesql任务与数据同步间任务依赖功能正常_hivesql初始化方式创建_自动依赖
**优先级**: P2
**前置条件**: 1）数据源信息：
a）数据源A：mysql数据源
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建hivesql任务A1，脚本如下：select * from table_B;【调度属性-任务间依赖】选择“自动依赖”，刷新
2. 修改脚本如下：select * from schema_B.table_B;【调度属性-任务间依赖】的自动依赖，刷新
3. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_B t1 left join schema_B.table_B t2 on t1.col=t2.col) t3 left join schema_B.table_C_1 t4 on t3.col=t4.col;【调度属性-任务间依赖】的自动依赖，刷新
4. 修改脚本如下：create table table_C_1 like table_B;【调度属性-任务间依赖】的自动依赖，刷新
5. 修改脚本如下：create table table_C_2 as select * from schema_B.table_B;【调度属性-任务间依赖】的自动依赖，刷新
6. 修改脚本如下：insert into schema_B.table_B select * from table_B;【调度属性-任务间依赖】的自动依赖，刷新
7. 修改脚本如下：insert overwrite table table_C_2 select t1.id,t2.name,t3.age from table_B t1 left join schema_B.table_B t2 on t1.col=t2.col;【调度属性-任务间依赖】的自动依赖，刷新
...
**预期**:
1. 1）自动依赖任务如下：
2. a）租户A，项目B下的数据同步任务A
3. 2）提交任务，任务提交成功，系统给出提交成功提示，状态更新为下一处理阶段
4. 1）自动依赖任务如下：
5. a）租户A，项目B下的数据同步任务B
6. b）租户A，项目A下的数据同步任务B
...
## 验证hivesql任务与数据同步间任务依赖功能正常_hivesql初始化方式创建_手动依赖_手动添加
**优先级**: P2
**前置条件**: 1）数据源信息：
a）数据源A：mysql数据源
b）数据源B：hive_B数据源（meta数据源）
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建hivesql任务A1，脚本如下：select * from table_B;【调度属性-任务间依赖】选择“手动依赖”，保存并提交
2. 修改脚本如下：select * from schema_B.table_B;保存并提交
3. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_B t1 left join schema_B.table_B t2 on t1.col=t2.col) t3 left join schema_B.table_C_1 t4 on t3.col=t4.col;保存并提交
4. 修改脚本如下：create table table_C_1 like table_B;保存并提交
5. 修改脚本如下：create table table_C_2 as select * from schema_B.table_B;保存并提交
6. 修改脚本如下：insert into schema_B.table_B select * from table_B;保存并提交
7. 修改脚本如下：insert overwrite table table_C_2 select t1.id,t2.name,t3.age from table_B t1 left join schema_B.table_B t2 on t1.col=t2.col;保存并提交
8. 修改脚本如下：select * from table_B;select * from schema_B.table_B;select * from (select t1.id,t2.name,t3.age from table_B t1 left join schema_B.table_B t2 on t1.col=t2.col) t3 left join schema_B.table_C_1 t4 on t3.col=t4.col;create table table_C_1 like table_B;create table table_C_2 as select * from schema_B.table_B;insert into schema_B.table_B select * from table_B;insert overwrite table table_C_2 select t1.id,t2.name,t3.age from table_B t1 left join schema_B.table_B t2 on t1.col=t2.col;保存并提交

**预期**:
1. 进入【任务依赖配置有误】弹窗：
2. 1）展示缺失上游依赖任务如下：
3. a）租户A，项目B下的数据同步任务A
4. 2）点击“仍要提交”：进入提交弹窗，确认后，任务提交成功
5. 进入【任务依赖配置有误】弹窗：
6. 1）展示缺失上游依赖任务如下：
7. a）租户A，项目B下的数据同步任务B
...
## 验证hivesql任务与数据同步间任务依赖功能正常_hivesql初始化方式创建_手动依赖_依赖推荐
**优先级**: P2
**前置条件**: 1）数据源信息：
a）数据源A：mysql数据源
b）数据源B：hive_B数据源（meta数据源）
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建hivesql任务A1，脚本如下：select * from table_B;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
2. 修改脚本如下：select * from schema_B.table_B;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
3. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_B t1 left join schema_B.table_B t2 on t1.col=t2.col) t3 left join schema_B.table_C_1 t4 on t3.col=t4.col;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
4. 修改脚本如下：create table table_C_1 like table_B;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
5. 修改脚本如下：create table table_C_2 as select * from schema_B.table_B;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
6. 修改脚本如下：insert into schema_B.table_B select * from table_B;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
7. 修改脚本如下：insert overwrite table table_C_2 select t1.id,t2.name,t3.age from table_B t1 left join schema_B.table_B t2 on t1.col=t2.col;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
8. 修改脚本如下：select * from table_B;select * from schema_B.table_B;select * from (select t1.id,t2.name,t3.age from table_B t1 left join schema_B.table_B t2 on t1.col=t2.col) t3 left join schema_B.table_C_1 t4 on t3.col=t4.col;create table table_C_1 like table_B;create table table_C_2 as select * from schema_B.table_B;insert into schema_B.table_B select * from table_B;insert overwrite table table_C_2 select t1.id,t2.name,t3.age from table_B t1 left join schema_B.table_B t2 on t1.col=t2.col;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
...
## 验证hivesql任务与数据同步间任务依赖功能正常_hivesql初始化方式不创建且不对接schema_自动依赖
**优先级**: P2
**前置条件**: 1）数据源信息：
a）数据源A：mysql数据源
b）数据源B：hive_B数据源（meta数据源）
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建hivesql任务A1，选择schema_C，脚本如下：select * from table_B;【调度属性-任务间依赖】选择“自动依赖”，刷新
2. 修改脚本如下：select * from schema_B.table_B;【调度属性-任务间依赖】的自动依赖，刷新
3. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_B t1 left join schema_B.table_B t2 on t1.col=t2.col) t3 left join schema_B.table_C_1 t4 on t3.col=t4.col;【调度属性-任务间依赖】的自动依赖，刷新
4. 修改脚本如下：create table table_C_1 like table_B;【调度属性-任务间依赖】的自动依赖，刷新
5. 修改脚本如下：create table table_C_2 as select * from schema_B.table_B;【调度属性-任务间依赖】的自动依赖，刷新
...
2. 2）提交任务，任务提交成功，系统给出提交成功提示，状态更新为下一处理阶段
3. 1）自动依赖任务如下：
4. a）租户A，项目B下的数据同步任务B
5. b）租户A，项目A下的数据同步任务B
6. 2）提交任务，任务提交成功，系统给出提交成功提示，状态更新为下一处理阶段
7. 1）自动依赖任务如下：
8. a）租户A，项目B下的数据同步任务B
9. b）租户A，项目A下的数据同步任务B
...
## 验证hivesql任务与数据同步间任务依赖功能正常_hivesql初始化方式不创建且不对接schema_手动依赖_手动添加
**优先级**: P2
**前置条件**: 1）数据源信息：
a）数据源A：mysql数据源
b）数据源B：hive_B数据源（meta数据源）
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建hivesql任务A1，选择schema_C，脚本如下：select * from table_B;【调度属性-任务间依赖】选择“手动依赖”，保存并提交
2. 修改脚本如下：select * from schema_B.table_B;保存并提交
3. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_B t1 left join schema_B.table_B t2 on t1.col=t2.col) t3 left join schema_B.table_C_1 t4 on t3.col=t4.col;保存并提交
4. 修改脚本如下：create table table_C_1 like table_B;保存并提交
5. 修改脚本如下：create table table_C_2 as select * from schema_B.table_B;保存并提交
...
## 验证hivesql任务与数据同步间任务依赖功能正常_hivesql初始化方式不创建且不对接schema_手动依赖_依赖推荐
**优先级**: P2
**前置条件**: 1）数据源信息：
a）数据源A：mysql数据源
b）数据源B：hive_B数据源（meta数据源）
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建hivesql任务A1，选择schema_C，脚本如下：select * from table_B;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
2. 修改脚本如下：select * from schema_B.table_B;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
3. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_B t1 left join schema_B.table_B t2 on t1.col=t2.col) t3 left join schema_B.table_C_1 t4 on t3.col=t4.col;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
4. 修改脚本如下：create table table_C_1 like table_B;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
5. 修改脚本如下：create table table_C_2 as select * from schema_B.table_B;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建hivesql任务C，脚本如下：select * from table_A;【调度属性-任务间依赖】选择“自动依赖”，查看依赖任务
2. 【调度属性-任务间依赖】切换为“手动依赖”，保存并提交
3. 【调度属性-任务间依赖】切换为“手动依赖”，“添加依赖”选择“依赖推荐”
4. 进入租户A下项目B的【数据开发-周期任务】页面，新建hivesql任务D，脚本如下：select * from table_B_1;...select * from table_B_50;【调度属性-任务间依赖】选择“自动依赖”，查看依赖任务
5. 【调度属性-任务间依赖】切换为“手动依赖”，保存并提交
6. 【调度属性-任务间依赖】切换为“手动依赖”，“添加依赖”选择“依赖推荐”

**预期**:
1. 1）自动依赖任务如下：
2. a）租户A，项目B下的hivesql任务A1-A50
3. 2）提交任务，任务提交成功，系统给出提交成功提示，状态更新为下一处理阶段
4. 进入【任务依赖配置有误】弹窗：
5. 1）展示缺失上游依赖任务如下：
6. a）租户A，项目B下的hivesql任务A1-A50
...
## 验证mysql任务间任务依赖功能正常_mysql初始化方式创建_自动依赖
**优先级**: P2
**前置条件**: 1）项目A、项目B的mysql引擎的初始化方式均为创建
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建mysql任务A1，脚本如下：select * from table_A;【调度属性-任务间依赖】选择“自动依赖”，刷新
2. 修改脚本如下：select * from table_B_1;【调度属性-任务间依赖】的自动依赖，刷新
3. 修改脚本如下：select * from table_B_2;【调度属性-任务间依赖】的自动依赖，刷新
4. 修改脚本如下：select * from schema_B.table_B_2;【调度属性-任务间依赖】的自动依赖，刷新
5. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_A t1 left join schema_B.table_B_1 t2 on t1.col=t2.col) t3 left join schema_B.table_B_2 t4 on t3.col=t4.col;【调度属性-任务间依赖】的自动依赖，刷新
...
## 验证mysql任务间任务依赖功能正常_mysql初始化方式创建_手动依赖_手动添加
**优先级**: P2
**前置条件**: 1）项目A、项目B的mysql引擎的初始化方式均为创建
2）mysql任务（租户A下，在项目A、项目B均配置以下任务且已提交）：
a）mysql任务A：create table table_A;
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建mysql任务A1，脚本如下：select * from table_A;【调度属性-任务间依赖】选择“手动依赖”，保存并提交
2. 修改脚本如下：select * from table_B_1;保存并提交
3. 修改脚本如下：select * from table_B_2;保存并提交
4. 修改脚本如下：select * from schema_B.table_B_2;保存并提交
5. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_A t1 left join schema_B.table_B_1 t2 on t1.col=t2.col) t3 left join schema_B.table_B_2 t4 on t3.col=t4.col;保存并提交
...
7. 修改脚本如下：create table table_C_2 as select * from table_A;保存并提交
8. 修改脚本如下：insert into schema_B.table_B_1 select * from table_A;保存并提交
9. 修改脚本如下：select * from table_A;select * from table_B_1;select * from table_B_2;select * from schema_B.table_B_2;select * from (select t1.id,t2.name,t3.age from table_A t1 left join schema_B.table_B_1 t2 on t1.col=t2.col) t3 left join schema_B.table_B_2 t4 on t3.col=t4.col;create table table_C_1 like table_A;create table table_C_2 as select * from table_A;insert into schema_B.table_B_1 select * from table_A;保存并提交
...
## 验证mysql任务间任务依赖功能正常_mysql初始化方式创建_手动依赖_依赖推荐
**优先级**: P2
**前置条件**: 1）项目A、项目B的mysql引擎的初始化方式均为创建
2）mysql任务（租户A下，在项目A、项目B均配置以下任务且已提交）：
a）mysql任务A：create table table_A;
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建mysql任务A1，脚本如下：select * from table_A;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
2. 修改脚本如下：select * from table_B_1;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
3. 修改脚本如下：select * from table_B_2;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
4. 修改脚本如下：select * from schema_B.table_B_2;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
5. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_A t1 left join schema_B.table_B_1 t2 on t1.col=t2.col) t3 left join schema_B.table_B_2 t4 on t3.col=t4.col;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
...
7. 修改脚本如下：create table table_C_2 as select * from table_A;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
8. 修改脚本如下：insert into schema_B.table_B_1 select * from table_A;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
9. 修改脚本如下：select * from table_A;select * from table_B_1;select * from table_B_2;select * from schema_B.table_B_2;select * from (select t1.id,t2.name,t3.age from table_A t1 left join schema_B.table_B_1 t2 on t1.col=t2.col) t3 left join schema_B.table_B_2 t4 on t3.col=t4.col;create table table_C_1 like table_A;create table table_C_2 as select * from table_A;insert into schema_B.table_B_1 select * from table_A;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
...
3. 2）未添加推荐依赖，保存并提交任务，进入【任务依赖配置有误】弹窗：
4. a）展示缺失上游依赖任务如下：
5. i）租户A，项目B下的mysql任务A
6. b）点击“仍要提交”：进入提交弹窗，确认后，任务提交成功
7. 1）依赖推荐任务如下：
8. a）租户A，项目B下的mysql任务D
9. 2）未添加推荐依赖，保存并提交任务，进入【任务依赖配置有误】弹窗：
10. a）展示缺失上游依赖任务如下：
...
## 验证mysql任务间任务依赖功能正常_mysql初始化方式不创建且不对接schema_自动依赖
**优先级**: P2
**前置条件**: 1）项目A、项目B的mysql引擎的初始化方式均为不创建且不对接schema
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建mysql任务A1，选择schema_C，脚本如下：select * from schema_B.table_A;【调度属性-任务间依赖】选择“自动依赖”，刷新
2. 修改脚本如下：select * from schema_B.table_B_1;【调度属性-任务间依赖】选择“自动依赖”，刷新
3. 修改脚本如下：select * from schema_B.table_B_2;【调度属性-任务间依赖】选择“自动依赖”，刷新
4. 修改脚本如下：select * from table_A;【调度属性-任务间依赖】选择“自动依赖”，刷新
5. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_A t1 left join schema_B.table_B_1 t2 on t1.col=t2.col) t3 left join schema_B.table_B_2 t4 on t3.col=t4.col;【调度属性-任务间依赖】选择“自动依赖”，刷新
...
## 验证mysql任务间任务依赖功能正常_mysql初始化方式不创建且不对接schema_手动依赖_手动添加
**优先级**: P2
**前置条件**: 1）项目A、项目B的mysql引擎的初始化方式均为不创建且不对接schema
2）mysql任务（租户A下，在项目A、项目B均配置以下任务且已提交，所选schema均为schema_C）：
a）mysql任务A：create table table_A;
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建mysql任务A1，选择schema_C，脚本如下：select * from schema_B.table_A;【调度属性-任务间依赖】选择“手动依赖”，保存并提交
2. 修改脚本如下：select * from schema_B.table_B_1;保存并提交
3. 修改脚本如下：select * from schema_B.table_B_2;保存并提交
4. 修改脚本如下：select * from table_A;保存并提交
5. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_A t1 left join schema_B.table_B_1 t2 on t1.col=t2.col) t3 left join schema_B.table_B_2 t4 on t3.col=t4.col;保存并提交
...
## 验证mysql任务间任务依赖功能正常_mysql初始化方式不创建且不对接schema_手动依赖_依赖推荐
**优先级**: P2
**前置条件**: 1）项目A、项目B的mysql引擎的初始化方式均为不创建且不对接schema
2）mysql任务（租户A下，在项目A、项目B均配置以下任务且已提交，所选schema均为schema_C）：
a）mysql任务A：create table table_A;
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建mysql任务A1，选择schema_C，脚本如下：select * from schema_B.table_A;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
2. 修改脚本如下：select * from schema_B.table_B_1;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
3. 修改脚本如下：select * from schema_B.table_B_2;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
4. 修改脚本如下：select * from table_A;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
5. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_A t1 left join schema_B.table_B_1 t2 on t1.col=t2.col) t3 left join schema_B.table_B_2 t4 on t3.col=t4.col;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
...
6. 2）未添加推荐依赖，保存并提交任务，进入【任务依赖配置有误】弹窗：
7. a）展示缺失上游依赖任务如下：
8. i）租户A，项目A下的mysql任务B、D
9. ii）租户A，项目B下的mysql任务B、D
10. b）点击“仍要提交”：进入提交弹窗，确认后，任务提交成功
11. 1）依赖推荐任务如下：
...
## 验证mysql任务与数据同步间任务依赖功能正常_mysql初始化方式创建_自动依赖
**优先级**: P2
**前置条件**: 1）数据源信息：
a）数据源A：mysql数据源
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建mysql任务A1，脚本如下：select * from table_B;【调度属性-任务间依赖】选择“自动依赖”，刷新
2. 修改脚本如下：select * from schema_B.table_B;【调度属性-任务间依赖】的自动依赖，刷新
3. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_B t1 left join schema_B.table_B t2 on t1.col=t2.col) t3 left join schema_B.table_C_1 t4 on t3.col=t4.col;【调度属性-任务间依赖】的自动依赖，刷新
4. 修改脚本如下：create table table_C_1 like table_B;【调度属性-任务间依赖】的自动依赖，刷新
5. 修改脚本如下：create table table_C_2 as select * from schema_B.table_B;【调度属性-任务间依赖】的自动依赖，刷新
6. 修改脚本如下：insert into schema_B.table_B select * from table_B;【调度属性-任务间依赖】的自动依赖，刷新
7. 修改脚本如下：select * from table_B;select * from schema_B.table_B;select * from (select t1.id,t2.name,t3.age from table_B t1 left join schema_B.table_B t2 on t1.col=t2.col) t3 left join schema_B.table_C_1 t4 on t3.col=t4.col;create table table_C_1 like table_B;create table table_C_2 as select * from schema_B.table_B;insert into schema_B.table_B select * from table_B;【调度属性-任务间依赖】的自动依赖，刷新
...
## 验证mysql任务与数据同步间任务依赖功能正常_mysql初始化方式创建_手动依赖_手动添加
**优先级**: P2
**前置条件**: 1）数据源信息：
a）数据源A：mysql数据源
b）数据源B：mysql_B数据源（meta数据源）
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建mysql任务A1，脚本如下：select * from table_B;【调度属性-任务间依赖】选择“手动依赖”，保存并提交
2. 修改脚本如下：select * from schema_B.table_B;保存并提交
3. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_B t1 left join schema_B.table_B t2 on t1.col=t2.col) t3 left join schema_B.table_C_1 t4 on t3.col=t4.col;保存并提交
4. 修改脚本如下：create table table_C_1 like table_B;保存并提交
5. 修改脚本如下：create table table_C_2 as select * from schema_B.table_B;保存并提交
6. 修改脚本如下：insert into schema_B.table_B select * from table_B;保存并提交
7. 修改脚本如下：select * from table_B;select * from schema_B.table_B;select * from (select t1.id,t2.name,t3.age from table_B t1 left join schema_B.table_B t2 on t1.col=t2.col) t3 left join schema_B.table_C_1 t4 on t3.col=t4.col;create table table_C_1 like table_B;create table table_C_2 as select * from schema_B.table_B;insert into schema_B.table_B select * from table_B;保存并提交
...
## 验证mysql任务与数据同步间任务依赖功能正常_mysql初始化方式创建_手动依赖_依赖推荐
**优先级**: P2
**前置条件**: 1）数据源信息：
a）数据源A：mysql数据源
b）数据源B：mysql_B数据源（meta数据源）
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建mysql任务A1，脚本如下：select * from table_B;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
2. 修改脚本如下：select * from schema_B.table_B;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
3. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_B t1 left join schema_B.table_B t2 on t1.col=t2.col) t3 left join schema_B.table_C_1 t4 on t3.col=t4.col;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
4. 修改脚本如下：create table table_C_1 like table_B;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
5. 修改脚本如下：create table table_C_2 as select * from schema_B.table_B;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
6. 修改脚本如下：insert into schema_B.table_B select * from table_B;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
7. 修改脚本如下：select * from table_B;select * from schema_B.table_B;select * from (select t1.id,t2.name,t3.age from table_B t1 left join schema_B.table_B t2 on t1.col=t2.col) t3 left join schema_B.table_C_1 t4 on t3.col=t4.col;create table table_C_1 like table_B;create table table_C_2 as select * from schema_B.table_B;insert into schema_B.table_B select * from table_B;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
...
## 验证mysql任务与数据同步间任务依赖功能正常_mysql初始化方式不创建且不对接schema_自动依赖
**优先级**: P2
**前置条件**: 1）数据源信息：
a）数据源A：mysql数据源
b）数据源B：mysql_B数据源（meta数据源）
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建mysql任务A1，选择schema_C，脚本如下：select * from table_B;【调度属性-任务间依赖】选择“自动依赖”，刷新
2. 修改脚本如下：select * from schema_B.table_B;【调度属性-任务间依赖】的自动依赖，刷新
3. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_B t1 left join schema_B.table_B t2 on t1.col=t2.col) t3 left join schema_B.table_C_1 t4 on t3.col=t4.col;【调度属性-任务间依赖】的自动依赖，刷新
4. 修改脚本如下：create table table_C_1 like table_B;【调度属性-任务间依赖】的自动依赖，刷新
5. 修改脚本如下：create table table_C_2 as select * from schema_B.table_B;【调度属性-任务间依赖】的自动依赖，刷新
...
## 验证mysql任务与数据同步间任务依赖功能正常_mysql初始化方式不创建且不对接schema_手动依赖_手动添加
**优先级**: P2
**前置条件**: 1）数据源信息：
a）数据源A：mysql数据源
b）数据源B：mysql_B数据源（meta数据源）
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建mysql任务A1，选择schema_C，脚本如下：select * from table_B;【调度属性-任务间依赖】选择“手动依赖”，保存并提交
2. 修改脚本如下：select * from schema_B.table_B;保存并提交
3. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_B t1 left join schema_B.table_B t2 on t1.col=t2.col) t3 left join schema_B.table_C_1 t4 on t3.col=t4.col;保存并提交
4. 修改脚本如下：create table table_C_1 like table_B;保存并提交
5. 修改脚本如下：create table table_C_2 as select * from schema_B.table_B;保存并提交
...
## 验证mysql任务与数据同步间任务依赖功能正常_mysql初始化方式不创建且不对接schema_手动依赖_依赖推荐
**优先级**: P2
**前置条件**: 1）数据源信息：
a）数据源A：mysql数据源
b）数据源B：mysql_B数据源（meta数据源）
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建mysql任务A1，选择schema_C，脚本如下：select * from table_B;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
2. 修改脚本如下：select * from schema_B.table_B;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
3. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_B t1 left join schema_B.table_B t2 on t1.col=t2.col) t3 left join schema_B.table_C_1 t4 on t3.col=t4.col;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
4. 修改脚本如下：create table table_C_1 like table_B;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
5. 修改脚本如下：create table table_C_2 as select * from schema_B.table_B;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建mysql任务C，脚本如下：select * from table_A;【调度属性-任务间依赖】选择“自动依赖”，查看依赖任务
2. 【调度属性-任务间依赖】切换为“手动依赖”，保存并提交
3. 【调度属性-任务间依赖】切换为“手动依赖”，“添加依赖”选择“依赖推荐”
4. 进入租户A下项目B的【数据开发-周期任务】页面，新建mysql任务D，脚本如下：select * from table_B_1;...select * from table_B_50;【调度属性-任务间依赖】选择“自动依赖”，查看依赖任务
5. 【调度属性-任务间依赖】切换为“手动依赖”，保存并提交
6. 【调度属性-任务间依赖】切换为“手动依赖”，“添加依赖”选择“依赖推荐”

**预期**:
1. 1）自动依赖任务如下：
2. a）租户A，项目B下的mysql任务A1-A50
3. 2）提交任务，任务提交成功，系统给出提交成功提示，状态更新为下一处理阶段
4. 进入【任务依赖配置有误】弹窗：
5. 1）展示缺失上游依赖任务如下：
6. a）租户A，项目B下的mysql任务A1-A50
7. 2）点击“仍要提交”：进入提交弹窗，确认后，任务提交成功
8. 1）依赖推荐任务如下：
...
## 验证dorissql任务间任务依赖功能正常_dorissql初始化方式创建_自动依赖
**优先级**: P1
**前置条件**: 1）项目A、项目B的dorissql引擎的初始化方式均为创建
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建dorissql任务A1，脚本如下：select * from table_A;【调度属性-任务间依赖】选择“自动依赖”，刷新
2. 修改脚本如下：select * from table_B_1;【调度属性-任务间依赖】的自动依赖，刷新
3. 修改脚本如下：select * from table_B_2;【调度属性-任务间依赖】的自动依赖，刷新
4. 修改脚本如下：select * from schema_B.table_B_2;【调度属性-任务间依赖】的自动依赖，刷新
5. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_A t1 left join schema_B.table_B_1 t2 on t1.col=t2.col) t3 left join schema_B.table_B_2 t4 on t3.col=t4.col;【调度属性-任务间依赖】的自动依赖，刷新
...
**预期**:
1. 1）自动依赖任务如下：
2. a）租户A，项目B下的dorissql任务A
3. 2）提交任务，任务提交成功，系统给出提交成功提示，状态更新为下一处理阶段
4. 1）自动依赖任务如下：
5. a）租户A，项目B下的dorissql任务D
6. 2）提交任务，任务提交成功，系统给出提交成功提示，状态更新为下一处理阶段
7. 1）自动依赖任务如下：
8. a）租户A，项目B下的dorissql任务C、E
9. 2）提交任务，任务提交成功，系统给出提交成功提示，状态更新为下一处理阶段
10. 1）自动依赖任务如下：空
11. 2）提交任务，任务提交成功，系统给出提交成功提示，状态更新为下一处理阶段
12. 1）自动依赖任务如下：
...
## 验证dorissql任务间任务依赖功能正常_dorissql初始化方式创建_手动依赖_手动添加
**优先级**: P1
**前置条件**: 1）项目A、项目B的dorissql引擎的初始化方式均为创建
2）dorissql任务（租户A下，在项目A、项目B均配置以下任务且已提交）：
a）dorissql任务A：create table table_A;
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建dorissql任务A1，脚本如下：select * from table_A;【调度属性-任务间依赖】选择“手动依赖”，保存并提交
2. 修改脚本如下：select * from table_B_1;保存并提交
3. 修改脚本如下：select * from table_B_2;保存并提交
4. 修改脚本如下：select * from schema_B.table_B_2;保存并提交
5. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_A t1 left join schema_B.table_B_1 t2 on t1.col=t2.col) t3 left join schema_B.table_B_2 t4 on t3.col=t4.col;保存并提交
...
## 验证dorissql任务间任务依赖功能正常_dorissql初始化方式创建_手动依赖_依赖推荐
**优先级**: P2
**前置条件**: 1）项目A、项目B的dorissql引擎的初始化方式均为创建
2）dorissql任务（租户A下，在项目A、项目B均配置以下任务且已提交）：
a）dorissql任务A：create table table_A;
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建dorissql任务A1，脚本如下：select * from table_A;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
2. 修改脚本如下：select * from table_B_1;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
3. 修改脚本如下：select * from table_B_2;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
4. 修改脚本如下：select * from schema_B.table_B_2;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
5. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_A t1 left join schema_B.table_B_1 t2 on t1.col=t2.col) t3 left join schema_B.table_B_2 t4 on t3.col=t4.col;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
...
3. 2）未添加推荐依赖，保存并提交任务，进入【任务依赖配置有误】弹窗：
4. a）展示缺失上游依赖任务如下：
5. i）租户A，项目B下的dorissql任务A
6. b）点击“仍要提交”：进入提交弹窗，确认后，任务提交成功
7. 1）依赖推荐任务如下：
8. a）租户A，项目B下的dorissql任务D
9. 2）未添加推荐依赖，保存并提交任务，进入【任务依赖配置有误】弹窗：
10. a）展示缺失上游依赖任务如下：
...
## 验证dorissql任务间任务依赖功能正常_dorissql初始化方式不创建且不对接schema_自动依赖
**优先级**: P2
**前置条件**: 1）项目A、项目B的dorissql引擎的初始化方式均为不创建且不对接schema
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建dorissql任务A1，选择schema_C，脚本如下：select * from schema_B.table_A;【调度属性-任务间依赖】选择“自动依赖”，刷新
2. 修改脚本如下：select * from schema_B.table_B_1;【调度属性-任务间依赖】选择“自动依赖”，刷新
3. 修改脚本如下：select * from schema_B.table_B_2;【调度属性-任务间依赖】选择“自动依赖”，刷新
4. 修改脚本如下：select * from table_A;【调度属性-任务间依赖】选择“自动依赖”，刷新
5. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_A t1 left join schema_B.table_B_1 t2 on t1.col=t2.col) t3 left join schema_B.table_B_2 t4 on t3.col=t4.col;【调度属性-任务间依赖】选择“自动依赖”，刷新
...
2. 2）提交任务，任务提交成功，系统给出提交成功提示，状态更新为下一处理阶段
3. 1）自动依赖任务如下：
4. a）租户A，项目A下的dorissql任务B、D
5. b）租户A，项目B下的dorissql任务B、D
6. 2）提交任务，任务提交成功，系统给出提交成功提示，状态更新为下一处理阶段
7. 1）自动依赖任务如下：
8. a）租户A，项目A下的dorissql任务C、E
9. b）租户A，项目B下的dorissql任务C、E
10. 2）提交任务，任务提交成功，系统给出提交成功提示，状态更新为下一处理阶段
11. 1）自动依赖任务如下：
...
## 验证dorissql任务间任务依赖功能正常_dorissql初始化方式不创建且不对接schema_手动依赖_手动添加
**优先级**: P1
**前置条件**: 1）项目A、项目B的dorissql引擎的初始化方式均为不创建且不对接schema
2）dorissql任务（租户A下，在项目A、项目B均配置以下任务且已提交，所选schema均为schema_C）：
a）dorissql任务A：create table table_A;
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建dorissql任务A1，选择schema_C，脚本如下：select * from schema_B.table_A;【调度属性-任务间依赖】选择“手动依赖”，保存并提交
2. 修改脚本如下：select * from schema_B.table_B_1;保存并提交
3. 修改脚本如下：select * from schema_B.table_B_2;保存并提交
4. 修改脚本如下：select * from table_A;保存并提交
5. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_A t1 left join schema_B.table_B_1 t2 on t1.col=t2.col) t3 left join schema_B.table_B_2 t4 on t3.col=t4.col;保存并提交
...
7. 进入【任务依赖配置有误】弹窗：
8. 1）展示缺失上游依赖任务如下：
9. a）租户A，项目A下的dorissql任务C、E
10. b）租户A，项目B下的dorissql任务C、E
11. 2）点击“仍要提交”：进入提交弹窗，确认后，任务提交成功
12. 进入【任务依赖配置有误】弹窗：
13. 1）展示缺失上游依赖任务如下：
...
## 验证dorissql任务间任务依赖功能正常_dorissql初始化方式不创建且不对接schema_手动依赖_依赖推荐
**优先级**: P1
**前置条件**: 1）项目A、项目B的dorissql引擎的初始化方式均为不创建且不对接schema
2）dorissql任务（租户A下，在项目A、项目B均配置以下任务且已提交，所选schema均为schema_C）：
a）dorissql任务A：create table table_A;
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建dorissql任务A1，选择schema_C，脚本如下：select * from schema_B.table_A;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
2. 修改脚本如下：select * from schema_B.table_B_1;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
3. 修改脚本如下：select * from schema_B.table_B_2;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
4. 修改脚本如下：select * from table_A;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
5. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_A t1 left join schema_B.table_B_1 t2 on t1.col=t2.col) t3 left join schema_B.table_B_2 t4 on t3.col=t4.col;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
...
6. 2）未添加推荐依赖，保存并提交任务，进入【任务依赖配置有误】弹窗：
7. a）展示缺失上游依赖任务如下：
8. i）租户A，项目A下的dorissql任务B、D
9. ii）租户A，项目B下的dorissql任务B、D
10. b）点击“仍要提交”：进入提交弹窗，确认后，任务提交成功
11. 1）依赖推荐任务如下：
...
## 验证dorissql任务与数据同步间任务依赖功能正常_dorissql初始化方式创建_自动依赖
**优先级**: P2
**前置条件**: 1）数据源信息：
a）数据源A：mysql数据源
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建dorissql任务A1，脚本如下：select * from table_B;【调度属性-任务间依赖】选择“自动依赖”，刷新
2. 修改脚本如下：select * from schema_B.table_B;【调度属性-任务间依赖】的自动依赖，刷新
3. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_B t1 left join schema_B.table_B t2 on t1.col=t2.col) t3 left join schema_B.table_C_1 t4 on t3.col=t4.col;【调度属性-任务间依赖】的自动依赖，刷新
4. 修改脚本如下：create table table_C_1 like table_B;【调度属性-任务间依赖】的自动依赖，刷新
5. 修改脚本如下：create table table_C_2 as select * from schema_B.table_B;【调度属性-任务间依赖】的自动依赖，刷新
...
## 验证dorissql任务与数据同步间任务依赖功能正常_dorissql初始化方式创建_手动依赖_手动添加
**优先级**: P1
**前置条件**: 1）数据源信息：
a）数据源A：mysql数据源
b）数据源B：doris_B数据源（meta数据源）
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建dorissql任务A1，脚本如下：select * from table_B;【调度属性-任务间依赖】选择“手动依赖”，保存并提交
2. 修改脚本如下：select * from schema_B.table_B;保存并提交
3. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_B t1 left join schema_B.table_B t2 on t1.col=t2.col) t3 left join schema_B.table_C_1 t4 on t3.col=t4.col;保存并提交
4. 修改脚本如下：create table table_C_1 like table_B;保存并提交
5. 修改脚本如下：create table table_C_2 as select * from schema_B.table_B;保存并提交
...
## 验证dorissql任务与数据同步间任务依赖功能正常_dorissql初始化方式创建_手动依赖_依赖推荐
**优先级**: P1
**前置条件**: 1）数据源信息：
a）数据源A：mysql数据源
b）数据源B：doris_B数据源（meta数据源）
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建dorissql任务A1，脚本如下：select * from table_B;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
2. 修改脚本如下：select * from schema_B.table_B;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
3. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_B t1 left join schema_B.table_B t2 on t1.col=t2.col) t3 left join schema_B.table_C_1 t4 on t3.col=t4.col;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
4. 修改脚本如下：create table table_C_1 like table_B;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
5. 修改脚本如下：create table table_C_2 as select * from schema_B.table_B;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
...
## 验证dorissql任务与数据同步间任务依赖功能正常_dorissql初始化方式不创建且不对接schema_自动依赖
**优先级**: P1
**前置条件**: 1）数据源信息：
a）数据源A：mysql数据源
b）数据源B：doris_B数据源（meta数据源）
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建dorissql任务A1，选择schema_C，脚本如下：select * from table_B;【调度属性-任务间依赖】选择“自动依赖”，刷新
2. 修改脚本如下：select * from schema_B.table_B;【调度属性-任务间依赖】的自动依赖，刷新
3. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_B t1 left join schema_B.table_B t2 on t1.col=t2.col) t3 left join schema_B.table_C_1 t4 on t3.col=t4.col;【调度属性-任务间依赖】的自动依赖，刷新
4. 修改脚本如下：create table table_C_1 like table_B;【调度属性-任务间依赖】的自动依赖，刷新
5. 修改脚本如下：create table table_C_2 as select * from schema_B.table_B;【调度属性-任务间依赖】的自动依赖，刷新
...
## 验证dorissql任务与数据同步间任务依赖功能正常_dorissql初始化方式不创建且不对接schema_手动依赖_手动添加
**优先级**: P1
**前置条件**: 1）数据源信息：
a）数据源A：mysql数据源
b）数据源B：doris_B数据源（meta数据源）
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建dorissql任务A1，选择schema_C，脚本如下：select * from table_B;【调度属性-任务间依赖】选择“手动依赖”，保存并提交
2. 修改脚本如下：select * from schema_B.table_B;保存并提交
3. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_B t1 left join schema_B.table_B t2 on t1.col=t2.col) t3 left join schema_B.table_C_1 t4 on t3.col=t4.col;保存并提交
4. 修改脚本如下：create table table_C_1 like table_B;保存并提交
5. 修改脚本如下：create table table_C_2 as select * from schema_B.table_B;保存并提交
...
## 验证dorissql任务与数据同步间任务依赖功能正常_dorissql初始化方式不创建且不对接schema_手动依赖_依赖推荐
**优先级**: P2
**前置条件**: 1）数据源信息：
a）数据源A：mysql数据源
b）数据源B：doris_B数据源（meta数据源）
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建dorissql任务A1，选择schema_C，脚本如下：select * from table_B;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
2. 修改脚本如下：select * from schema_B.table_B;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
3. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_B t1 left join schema_B.table_B t2 on t1.col=t2.col) t3 left join schema_B.table_C_1 t4 on t3.col=t4.col;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
4. 修改脚本如下：create table table_C_1 like table_B;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
5. 修改脚本如下：create table table_C_2 as select * from schema_B.table_B;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
...
## 验证dorissql任务依赖功能正常_切换doris运行集群_自动依赖
**优先级**: P1
**前置条件**: 1）dorissql任务（租户A下，在项目B均配置以下任务且已提交）：
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建dorissql任务A1（运行集群为doris_A），脚本如下：select * from table_A;select * from schema_B.table_A;select * from table_C_1;select * from schema_B.table_C_1;【调度属性-任务间依赖】选择“自动依赖”，点击“刷新”
2. 运行集群切换至doris_B
3. 修改前置dorissql任务F的运行集群为“doris_A”，保存并提交
4. 进入dorissql任务A1任务的【调度属性-任务间依赖】页面，点击“刷新”

**预期**:
1. 1）自动依赖任务如下：
2. a）租户A，项目B下的dorissql任务A、B、C、D
3. 2）提交任务，任务提交成功，系统给出提交成功提示，状态更新为下一处理阶段
4. 【调度属性-任务间依赖】刷新自动依赖任务：
5. 1）自动依赖任务如下：
...
## 验证dorissql任务依赖功能正常_切换doris运行集群_手动依赖_手动添加
**优先级**: P1
**前置条件**: 1）dorissql任务（租户A下，在项目B均配置以下任务且已提交）：
a）dorissql任务A（doris_A引擎初始化方式为创建）：create table table_A;
b）dorissql任务B（doris_A引擎初始化方式为创建）：create table schema_B.table_A;
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建dorissql任务A1（运行集群为doris_A），脚本如下：select * from table_A;select * from schema_B.table_A;select * from table_C_1;select * from schema_B.table_C_1;【调度属性-任务间依赖】选择“自动依赖”，保存并提交
2. 运行集群切换至doris_B，保存并提交任务
3. 修改前置dorissql任务F的运行集群为“doris_A”，保存并提交
4. 进入dorissql任务A1任务的【调度属性-任务间依赖】页面，保存并提交

**预期**:
1. 进入【任务依赖配置有误】弹窗：
2. 1）展示缺失上游依赖任务如下：
3. a）租户A，项目B下的dorissql任务A、B、C、D
4. 2）点击“仍要提交”：进入提交弹窗，确认后，任务提交成功
5. 进入【任务依赖配置有误】弹窗：
6. 1）展示缺失上游依赖任务如下：
...
## 验证dorissql任务依赖功能正常_切换doris运行集群_手动依赖_依赖推荐
**优先级**: P2
**前置条件**: 1）dorissql任务（租户A下，在项目B均配置以下任务且已提交）：
a）dorissql任务A（doris_A引擎初始化方式为创建）：create table table_A;
b）dorissql任务B（doris_A引擎初始化方式为创建）：create table schema_B.table_A;
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建dorissql任务A1（运行集群为doris_A），脚本如下：select * from table_A;select * from schema_B.table_A;select * from table_C_1;select * from schema_B.table_C_1;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
2. 运行集群切换至doris_B，【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
3. 修改前置dorissql任务F的运行集群为“doris_A”，保存并提交
4. 进入dorissql任务A1任务的【调度属性-任务间依赖】页面，选择“手动依赖”，“添加依赖”选择“依赖推荐”
...
**步骤**:
1. 进入租户A下项目B的【数据开发-周期任务】页面，新建dorissql任务C，脚本如下：select * from table_A;【调度属性-任务间依赖】选择“自动依赖”，查看依赖任务
2. 【调度属性-任务间依赖】切换为“手动依赖”，保存并提交
3. 【调度属性-任务间依赖】切换为“手动依赖”，“添加依赖”选择“依赖推荐”
4. 进入租户A下项目B的【数据开发-周期任务】页面，新建dorissql任务D，脚本如下：select * from table_B_1;...select * from table_B_50;【调度属性-任务间依赖】选择“自动依赖”，查看依赖任务
5. 【调度属性-任务间依赖】切换为“手动依赖”，保存并提交
6. 【调度属性-任务间依赖】切换为“手动依赖”，“添加依赖”选择“依赖推荐”

**预期**:
1. 1）自动依赖任务如下：
2. a）租户A，项目B下的dorissql任务A1-A50
3. 2）提交任务，任务提交成功，系统给出提交成功提示，状态更新为下一处理阶段
4. 进入【任务依赖配置有误】弹窗：
5. 1）展示缺失上游依赖任务如下：
6. a）租户A，项目B下的dorissql任务A1-A50
7. 2）点击“仍要提交”：进入提交弹窗，确认后，任务提交成功
8. 1）依赖推荐任务如下：
...
**步骤**:
1. 进入租户A下项目B的【数据开发-手动任务】页面，新建数据同步任务A1，源表：数据源A下schema_A.table_B，目标表：数据源A下schema_A.table_C，其余内容均正常填写，保存并提交
2. 进入租户A下项目B的【数据开发-手动任务】页面，新建数据同步任务B1，源表：数据源A下schema_B.table_A，目标表：数据源A下schema_A.table_C，其余内容均正常填写，保存并提交
3. 进入租户A下项目B的【数据开发-手动任务】页面，新建数据同步任务C1，源表：数据源A下schema_B.table_B，目标表：数据源A下schema_A.table_C，其余内容均正常填写，保存并提交
...
**步骤**:
1. 进入租户A下项目B的【数据开发-手动任务】页面，新建数据同步任务D1，源表：数据源B下schema_A.table_A，目标表：数据源A下schema_A.table_C，其余内容均正常填写，保存并提交
2. 进入租户A下项目B的【数据开发-手动任务】页面，新建数据同步任务E1，源表：数据源B下schema_A.table_B，目标表：数据源A下schema_A.table_C，其余内容均正常填写，保存并提交
3. 进入租户A下项目B的【数据开发-手动任务】页面，新建数据同步任务F1，源表：数据源B下schema_B.table_A，目标表：数据源A下schema_A.table_C，其余内容均正常填写，保存并提交
4. 进入租户A下项目B的【数据开发-手动任务】页面，新建数据同步任务G1，源表：数据源B下schema_B.table_B，目标表：数据源A下schema_A.table_C，其余内容均正常填写，保存并提交
5. 进入租户A下项目B的【数据开发-手动任务】页面，新建数据同步任务H1，源表：数据源C下schema_A.table_B，目标表：数据源A下schema_A.table_C，其余内容均正常填写，保存并提交
...
**步骤**:
1. 进入租户A下项目B的【数据开发-手动任务】页面，新建数据同步任务A，源表：数据源A下schema_meta.table_A，其余内容均正常填写，保存并提交
2. 进入租户A下项目B的【数据开发-手动任务】页面，新建数据同步任务B，源表：数据源A下schema_B.table_A，其余内容均正常填写，保存并提交
3. 进入租户A下项目B的【数据开发-手动任务】页面，新建数据同步任务C，源表：数据源A下table_B，其余内容均正常填写，保存并提交
4. 进入租户A下项目B的【数据开发-手动任务】页面，新建数据同步任务D，源表：数据源B下schema_B.table_A，其余内容均正常填写，保存并提交
5. 进入租户A下项目B的【数据开发-手动任务】页面，新建数据同步任务E，源表：数据源C下schema_B.table_A，其余内容均正常填写，保存并提交
...
1. 进入租户A下项目B的【数据开发-手动任务】页面，新建数据同步任务A，源表：数据源A下schema_meta.table_A，其余内容均正常填写，保存并提交
2. 进入租户A下项目B的【数据开发-手动任务】页面，新建数据同步任务B，源表：数据源A下schema_B.table_A，其余内容均正常填写，保存并提交
3. 进入租户A下项目B的【数据开发-手动任务】页面，新建数据同步任务D，源表：数据源B下schema_B.table_A，其余内容均正常填写，保存并提交
4. 进入租户A下项目B的【数据开发-手动任务】页面，新建数据同步任务E，源表：数据源A下schema_B.table_A，其余内容均正常填写，保存并提交
...
**步骤**:
1. 进入租户A下项目B的【数据开发-手动任务】页面，新建dorissql任务A1，脚本如下：select * from table_A;【调度属性-任务间依赖】选择“自动依赖”，刷新
2. 修改脚本如下：select * from table_B_1;【调度属性-任务间依赖】的自动依赖，刷新
3. 修改脚本如下：select * from table_B_2;【调度属性-任务间依赖】的自动依赖，刷新
4. 修改脚本如下：select * from schema_B.table_B_2;【调度属性-任务间依赖】的自动依赖，刷新
5. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_A t1 left join schema_B.table_B_1 t2 on t1.col=t2.col) t3 left join schema_B.table_B_2 t4 on t3.col=t4.col;【调度属性-任务间依赖】的自动依赖，刷新
...
**步骤**:
1. 进入租户A下项目B的【数据开发-手动任务】页面，新建dorissql任务A1，脚本如下：select * from table_A;【调度属性-任务间依赖】选择“手动依赖”，保存并提交
2. 修改脚本如下：select * from table_B_1;保存并提交
3. 修改脚本如下：select * from table_B_2;保存并提交
4. 修改脚本如下：select * from schema_B.table_B_2;保存并提交
5. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_A t1 left join schema_B.table_B_1 t2 on t1.col=t2.col) t3 left join schema_B.table_B_2 t4 on t3.col=t4.col;保存并提交
...
**步骤**:
1. 进入租户A下项目B的【数据开发-手动任务】页面，新建dorissql任务A1，脚本如下：select * from table_A;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
2. 修改脚本如下：select * from table_B_1;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
3. 修改脚本如下：select * from table_B_2;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
4. 修改脚本如下：select * from schema_B.table_B_2;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
5. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_A t1 left join schema_B.table_B_1 t2 on t1.col=t2.col) t3 left join schema_B.table_B_2 t4 on t3.col=t4.col;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
...
**步骤**:
1. 进入租户A下项目B的【数据开发-手动任务】页面，新建dorissql任务A1，选择schema_C，脚本如下：select * from schema_B.table_A;【调度属性-任务间依赖】选择“自动依赖”，刷新
2. 修改脚本如下：select * from schema_B.table_B_1;【调度属性-任务间依赖】选择“自动依赖”，刷新
3. 修改脚本如下：select * from schema_B.table_B_2;【调度属性-任务间依赖】选择“自动依赖”，刷新
4. 修改脚本如下：select * from table_A;【调度属性-任务间依赖】选择“自动依赖”，刷新
5. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_A t1 left join schema_B.table_B_1 t2 on t1.col=t2.col) t3 left join schema_B.table_B_2 t4 on t3.col=t4.col;【调度属性-任务间依赖】选择“自动依赖”，刷新
...
**步骤**:
1. 进入租户A下项目B的【数据开发-手动任务】页面，新建dorissql任务A1，选择schema_C，脚本如下：select * from schema_B.table_A;【调度属性-任务间依赖】选择“手动依赖”，保存并提交
2. 修改脚本如下：select * from schema_B.table_B_1;保存并提交
3. 修改脚本如下：select * from schema_B.table_B_2;保存并提交
4. 修改脚本如下：select * from table_A;保存并提交
5. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_A t1 left join schema_B.table_B_1 t2 on t1.col=t2.col) t3 left join schema_B.table_B_2 t4 on t3.col=t4.col;保存并提交
...
**步骤**:
1. 进入租户A下项目B的【数据开发-手动任务】页面，新建dorissql任务A1，选择schema_C，脚本如下：select * from schema_B.table_A;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
2. 修改脚本如下：select * from schema_B.table_B_1;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
3. 修改脚本如下：select * from schema_B.table_B_2;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
4. 修改脚本如下：select * from table_A;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
5. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_A t1 left join schema_B.table_B_1 t2 on t1.col=t2.col) t3 left join schema_B.table_B_2 t4 on t3.col=t4.col;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
...
**步骤**:
1. 进入租户A下项目B的【数据开发-手动任务】页面，新建dorissql任务A1，脚本如下：select * from table_B;【调度属性-任务间依赖】选择“自动依赖”，刷新
2. 修改脚本如下：select * from schema_B.table_B;【调度属性-任务间依赖】的自动依赖，刷新
3. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_B t1 left join schema_B.table_B t2 on t1.col=t2.col) t3 left join schema_B.table_C_1 t4 on t3.col=t4.col;【调度属性-任务间依赖】的自动依赖，刷新
4. 修改脚本如下：create table table_C_1 like table_B;【调度属性-任务间依赖】的自动依赖，刷新
5. 修改脚本如下：create table table_C_2 as select * from schema_B.table_B;【调度属性-任务间依赖】的自动依赖，刷新
...
**步骤**:
1. 进入租户A下项目B的【数据开发-手动任务】页面，新建dorissql任务A1，脚本如下：select * from table_B;【调度属性-任务间依赖】选择“手动依赖”，保存并提交
2. 修改脚本如下：select * from schema_B.table_B;保存并提交
3. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_B t1 left join schema_B.table_B t2 on t1.col=t2.col) t3 left join schema_B.table_C_1 t4 on t3.col=t4.col;保存并提交
4. 修改脚本如下：create table table_C_1 like table_B;保存并提交
5. 修改脚本如下：create table table_C_2 as select * from schema_B.table_B;保存并提交
...
**步骤**:
1. 进入租户A下项目B的【数据开发-手动任务】页面，新建dorissql任务A1，脚本如下：select * from table_B;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
2. 修改脚本如下：select * from schema_B.table_B;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
3. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_B t1 left join schema_B.table_B t2 on t1.col=t2.col) t3 left join schema_B.table_C_1 t4 on t3.col=t4.col;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
4. 修改脚本如下：create table table_C_1 like table_B;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
5. 修改脚本如下：create table table_C_2 as select * from schema_B.table_B;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
...
**步骤**:
1. 进入租户A下项目B的【数据开发-手动任务】页面，新建dorissql任务A1，选择schema_C，脚本如下：select * from table_B;【调度属性-任务间依赖】选择“自动依赖”，刷新
2. 修改脚本如下：select * from schema_B.table_B;【调度属性-任务间依赖】的自动依赖，刷新
3. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_B t1 left join schema_B.table_B t2 on t1.col=t2.col) t3 left join schema_B.table_C_1 t4 on t3.col=t4.col;【调度属性-任务间依赖】的自动依赖，刷新
4. 修改脚本如下：create table table_C_1 like table_B;【调度属性-任务间依赖】的自动依赖，刷新
5. 修改脚本如下：create table table_C_2 as select * from schema_B.table_B;【调度属性-任务间依赖】的自动依赖，刷新
...
**步骤**:
1. 进入租户A下项目B的【数据开发-手动任务】页面，新建dorissql任务A1，选择schema_C，脚本如下：select * from table_B;【调度属性-任务间依赖】选择“手动依赖”，保存并提交
2. 修改脚本如下：select * from schema_B.table_B;保存并提交
3. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_B t1 left join schema_B.table_B t2 on t1.col=t2.col) t3 left join schema_B.table_C_1 t4 on t3.col=t4.col;保存并提交
4. 修改脚本如下：create table table_C_1 like table_B;保存并提交
5. 修改脚本如下：create table table_C_2 as select * from schema_B.table_B;保存并提交
...
**步骤**:
1. 进入租户A下项目B的【数据开发-手动任务】页面，新建dorissql任务A1，选择schema_C，脚本如下：select * from table_B;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
2. 修改脚本如下：select * from schema_B.table_B;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
3. 修改脚本如下：select * from (select t1.id,t2.name,t3.age from table_B t1 left join schema_B.table_B t2 on t1.col=t2.col) t3 left join schema_B.table_C_1 t4 on t3.col=t4.col;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
4. 修改脚本如下：create table table_C_1 like table_B;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
5. 修改脚本如下：create table table_C_2 as select * from schema_B.table_B;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
...
**步骤**:
1. 进入租户A下项目B的【数据开发-手动任务】页面，新建dorissql任务A1（运行集群为doris_A），脚本如下：select * from table_A;select * from schema_B.table_A;select * from table_C_1;select * from schema_B.table_C_1;【调度属性-任务间依赖】选择“自动依赖”，点击“刷新”
2. 运行集群切换至doris_B
3. 修改前置dorissql任务F的运行集群为“doris_A”，运行后，保存并提交
4. 进入dorissql任务A1任务的【调度属性-任务间依赖】页面，点击“刷新”
...
**步骤**:
1. 进入租户A下项目B的【数据开发-手动任务】页面，新建dorissql任务A1（运行集群为doris_A），脚本如下：select * from table_A;select * from schema_B.table_A;select * from table_C_1;select * from schema_B.table_C_1;【调度属性-任务间依赖】选择“自动依赖”，保存并提交
2. 运行集群切换至doris_B，保存并提交任务
3. 修改前置dorissql任务F的运行集群为“doris_A”，保存并提交
4. 进入dorissql任务A1任务的【调度属性-任务间依赖】页面，保存并提交
...
**步骤**:
1. 进入租户A下项目B的【数据开发-手动任务】页面，新建dorissql任务A1（运行集群为doris_A），脚本如下：select * from table_A;select * from schema_B.table_A;select * from table_C_1;select * from schema_B.table_C_1;【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
2. 运行集群切换至doris_B，【调度属性-任务间依赖】选择“手动依赖”，“添加依赖”选择“依赖推荐”
3. 修改前置dorissql任务F的运行集群为“doris_A”，保存并提交
4. 进入dorissql任务A1任务的【调度属性-任务间依赖】页面，选择“手动依赖”，“添加依赖”选择“依赖推荐”
...
**步骤**:
1. 进入租户A下项目B的【数据开发-手动任务】页面，新建dorissql任务C，脚本如下：select * from table_A;【调度属性-任务间依赖】选择“自动依赖”，查看依赖任务
2. 【调度属性-任务间依赖】切换为“手动依赖”，保存并提交
3. 【调度属性-任务间依赖】切换为“手动依赖”，“添加依赖”选择“依赖推荐”
4. 进入租户A下项目B的【数据开发-手动任务】页面，新建dorissql任务D，脚本如下：select * from table_B_1;...select * from table_B_50;【调度属性-任务间依赖】选择“自动依赖”，查看依赖任务
5. 【调度属性-任务间依赖】切换为“手动依赖”，保存并提交
6. 【调度属性-任务间依赖】切换为“手动依赖”，“添加依赖”选择“依赖推荐”
