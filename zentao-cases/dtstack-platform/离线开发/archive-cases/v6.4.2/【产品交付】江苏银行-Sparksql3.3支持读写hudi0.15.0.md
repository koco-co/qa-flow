...
## 验证函数管理-spark3.3版本hudi系统函数正常使用
**优先级**: P2
**前置条件**: 控制台计算引擎已配置spark3.3版本
创建项目时引入hadoop组件
...
**步骤**:
1. 进入数据开发模块，新建3.3版本的spark sql周期任务
2. 新建3.3版本的spark sql任务，点击确认，输入hudi系统函数，点击保存
3. 点击临时运行

**预期**:
1. 成功进入数据开发模块，新建3.3版本的spark sql周期任务页面，页面内容正常加载显示，无报错
2. 任务创建、保存成功，系统给出保存成功提示，配置/数据已持久化，页面更新为最新状态
3. 任务正常运行，运行结果符合预期

---

## 验证补数据运行-spark3.3版本任务补数据运行正常运行
**优先级**: P2
...
**步骤**:
1. 进入数据开发模块，新建3.3版本的spark sql临时查询
2. 输入sql语句：前置条件内sql
3. 点击保存
4. 点击临时运行
...
**步骤**:
1. 进入数据开发模块，新建3.3版本的spark sql手动任务
2. 输入sql语句：前置条件内sql
3. 点击保存
4. 点击临时运行
