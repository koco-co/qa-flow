...
## 验证「报告详情」-查看详情功能正常
**优先级**: P2
**前置条件**: 1) 已在离线平台中创建Hive/Doris/Spark源的车辆信息表, 创建并执行DorisSQL语句:
...
2. 点击【已生成报告】页签, 选择校验成功的报告记录, 点击【报告详情】按钮
3. 点击操作中的【查看详情】

**预期**:
1. 成功进入【已配置报告】页面
2. 跳转到【质量报告】详情页面, 【规则校验明细-多表规则-质检结果】中存在【校验未通过】的记录
3. 支持查看明细数据，展示内容和任务实例模块的查看明细弹窗/抽屉保持一致

---

## 验证「报告详情」-查看日志功能正常
**优先级**: P2
**前置条件**: 1) 已在离线平台中创建Hive/Doris/Spark源的车辆信息表, 创建并执行SparkSQL语句:
...
**步骤**:
1. 进入离线开发-周期任务, 创建并执行Hive SQL任务: DROP TABLE car_compare02;
2. 进入【数据资产】-【数据质量】-【质量报告】页面
3. 在【已配置报告】中新建报告, 规则范围默认【全部】,报告周期为【一次性】, 并配置car_compare02表, 点击【确定】
4. 点击【已生成报告】页签, 进入报告详情, 点击操作中的【查看日志】

**预期**:
1. 源表删除成功, 但是元数据表仍存在于资产平台
2. 成功进入【已配置报告】页面
3. 1) 表单提交成功，系统给出提交成功提示，状态更新为下一处理阶段
2) 质量报告校验失败
4. 支持查看日志数据，和任务实例模块的查看日志内容保持一致
...
2. 点击【已生成报告】页签, 选择校验成功的报告记录, 点击【报告详情】按钮
3. UI CHECK
4. 点击质量评估汇总（car_compare01
-delivery_time）中的【车型】筛选图标
5. 勾选【H53a】 , 点击【确定】
6. 重置后, 再次点击【车型】筛选图标，勾选【H53a】和【H53b】 , 点击【确定】
...
3. 点击【车型】筛选图标
4. 勾选【H53a】 , 点击【确定】
5. 重置后, 再次点击【车型】筛选图标，勾选【H53a】和【H53b】 , 点击【确定】

**预期**:
1. 成功进入【已配置报告】页面
2. 跳转到【质量报告】详情页面
3. 1) 支持选择car_config字段的所有枚举2) 按钮: 重置/确定, 重置默认置灰
4. 显示【车型】为【H53a】的记录
5. 显示【车型】为【H53a】或【H53b】的记录

---

## 验证「报告详情」-页面&功能正常(Hive 2.x)
**优先级**: P3
**前置条件**: 1) 已在离线平台中创建Hive/Doris/Spark源的车辆信息表, 创建并执行HiveSQL语句:
...
2. 点击【已生成报告】页签, 选择校验成功的报告记录, 点击【报告详情】按钮
3. 点击【下载报告】
4. 点击【已生成报告】页签, 选择校验失败的报告记录, 点击【报告详情】按钮
5. 点击【下载报告】

**预期**:
1. 成功进入【已配置报告】页面
2. 跳转到【质量报告】详情页面
3. 下载的报告中包含: 1个pdf文件和1个明细数据（异常数据）压缩包和多个xlsx文件, 格式如下:1) pdf文件命名为“xxx(报告名称)+20250819（生成日期）.pdf”，2) 压缩包命名为“xxx(报告名称)+异常数据+20250819（生成日期）.zip”3) 每个单独的规则生成一张明细数据表格，表格命名为“xxx(规则名称)+异常数据.xlsx”
4. 跳转到【质量报告】详情页面
5. 校验失败的日志无需下载
...
1. 成功进入【已配置报告】页面
2. 跳转到【质量报告】详情页面
3. 质量评估汇总(car_compare02--delivery_time=2025-10-01)
1) 不显示车辆数
2) 不显示【车辆信息汇总】表格
