...
**优先级**: P3
**前置条件**: CREATE TABLE `ods_gate_common_rebate_ref_code_df` (
...
**步骤**:
1. 进入【数据资产】-【数据模型】页面
2. 点击【新建表】，数据源选择Doris类型，输入必填项，选择表类型为明细表，点击下一步
3. 点击建表语句模式，输入前提条件内的建表SQL，点击解析
4. 点击生成建表语句
5. 点击建表
6. 点击表名
...
2. 跳转【字段配置】页面
3. 字段解析正确，内容与预期完全一致，无异常或错误
4. 建表语句生成正确，内容与预期完全一致，无异常或错误
5. 系统提示建表成功，数据库中成功创建该表，表结构与配置字段一致
6. 跳转到数据地图详情页
...
**优先级**: P1
**前置条件**: CREATE TABLE IF NOT EXISTS example_db.composite_partitioned_table
(
id BIGINT,
name VARCHAR(50),
age INT,
...
**优先级**: P3
**前置条件**: CREATE TABLE IF NOT EXISTS example_db.list_partitioned_table
(
id BIGINT,
name VARCHAR(50),
department_id INT,
...
**优先级**: P3
**前置条件**: CREATE TABLE IF NOT EXISTS example_db.date_range_partitioned_table
(
id BIGINT,
name VARCHAR(50),
age INT,
...
**优先级**: P3
**前置条件**: CREATE TABLE test_dynamic_partition(
order_id    BIGINT,
create_dt   DATE,
username    VARCHAR(20)
)
...
**优先级**: P3
**前置条件**: CREATE TABLE demo.hash_bucket_tbl(
oid         BIGINT,
dt          DATE,
region      VARCHAR(10),
amount      INT
...
**优先级**: P2
**前置条件**: CREATE TABLE IF NOT EXISTS example_tbl_agg
(
...
**步骤**:
1. 进入【数据资产】-【数据模型】页面
2. 点击【新建表】，数据源选择Doris类型，输入必填项，选择表类型为聚合表，点击下一步
3. 点击建表语句模式，输入前提条件内的建表SQL，点击解析
4. 点击生成建表语句
5. 点击建表
6. 点击表名
...
**优先级**: P2
**前置条件**: CREATE TABLE IF NOT EXISTS example_tbl_unique
(
user_id         LARGEINT        NOT NULL,
...
**步骤**:
1. 进入【数据资产】-【数据模型】页面
2. 点击【新建表】，数据源选择Doris类型，输入必填项，选择表类型为主键表，点击下一步
3. 点击建表语句模式，输入前提条件内的建表SQL，点击解析
4. 点击生成建表语句
5. 点击建表
6. 点击表名
...
**优先级**: P1
**前置条件**: CREATE TABLE IF NOT EXISTS example_tbl_duplicate
(
...
**步骤**:
1. 进入【数据资产】-【数据模型】页面
2. 点击【新建表】，数据源选择Doris类型，输入必填项，点击下一步

**预期**:
1. 成功进入目标页面，页面内容正常加载显示，无报错
2. 跳转【字段配置】页面，页面新增【建表语句模式】tab按钮
