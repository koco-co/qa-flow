...
**步骤**:
1. 进入【数据资产-数据质量】，新建单表校验，选择表doris2
2. 1）执行并提交离线任务；
2）查看对应表血缘关系

**预期**:
1. 成功进入【数据资产-数据质量】，新建单表校验，选择表doris2页面，页面内容正常加载显示，无报错
2. 表级血缘/字段级血缘正确，内容与预期完全一致，无异常或错误
...
1. 成功进入【数据资产-数据质量】，新建单表校验，选择表doris2页面，页面内容正常加载显示，无报错
2. 表级血缘关系正确：
3. 字段级血缘关系正确，内容与预期完全一致，无异常或错误
4. 表级血缘和字段级血缘正常，功能行为符合预期，无报错或异常
...
**步骤**:
1. 进入离线，执行前置中的spark建表语句
2. 进入资产数据地图，搜索表test_001
3. 查看表详情

**预期**:
1. 系统提示建表成功，数据库中成功创建该表，表结构与配置字段一致
2. 能搜索到新建的表
3. 信息正确，内容与预期完全一致，无异常或错误

---

## 验证【元数据同步】_【常规表】_【全类型字段】同步正常
**优先级**: P2
**前置条件**: 已存在包含全类型字段的表A，DDL/DML如下
...
**预期**:
1. 成功进入【数据资产-数据质量】，新建单表校验，选择表doris2页面，页面内容正常加载显示，无报错
2. 表同步成功， 且表详情信息展示正确， 数据预览正确，内容与预期完全一致，无异常或错误
...
**步骤**:
1. 进入【数据资产-数据质量】，新建单表校验，选择表doris2
2. 新增【元数据同步任务】，选择spark的数据源，选择库表，库表选择情况见前置条件，点击【临时运行】
3. 结束后查看同步结果
4. 进入数据地图首页查看
5. 在数据地图查看表
...
2. 新增一条元数据同步实例，运行中
3. 同步成功，同步结果与前置结果一致
4. 数据源类型表数量及数据表数量统计新增正确，内容与预期完全一致，无异常或错误
5. 存在元数据同步选择的表，表信息正确，内容与预期完全一致，无异常或错误

---

## 验证【联动】验证质量和离线绑定，多规则包-【表行数-弱规则-校验不通过】-校验结果正确
**优先级**: P2
**前置条件**: 1、doris数据源已存在表
建表语句：
...
2. 等待离线周期实例运行，进入【数据资产-数据质量-任务实例查询】
3. 进入【离线开发-运维中心-周期任务实例】
4. 进入【离线开发-运维中心-周期任务】，对doris2doris任务补当前数据及下游，查看结果

**预期**:
1. 系统提示新建成功，列表顶部出现新建的记录，记录内容与填写一致
2. 1）页面生成一条实例，校验未通过
2）查看明细展示doris1中的三条数据
3. 离线任务运行成功，状态更新为【成功】，运行/执行结果符合预期，日志无报错
4. 1）补数据质量工作流中有3个规则包，任务都运行成功，状态更新为【成功】，运行/执行结果符合预期，日志无报错
2）质量生成一条实例，校验未通过

---

## 验证【联动】验证质量和离线绑定，多规则包-【表行数-校验通过】-校验结果正确
**优先级**: P2
**前置条件**: 1、doris数据源已存在表
建表语句：
drop table if exists doris1;
...
## 验证【联动】验证质量和离线绑定，单规则包-【表行数-弱规则-校验不通过】结果正确
**优先级**: P2
**前置条件**: 1、doris数据源已存在表
建表语句：
drop table if exists doris1;
...
绑定任务：前置条件doris2doris
3. 等待离线实例运行，查看质量实例
4. 运行结束后，查看离线和质量实例的运行结果
5. 进入【离线开发-运维中心-周期任务】，对doris2doris任务补当前数据及下游，查看结果

**预期**:
1. 成功进入【数据资产-数据质量】，新建单表校验，选择表doris2页面，页面内容正常加载显示，无报错
2. 系统提示新建成功，列表顶部出现新建的记录，记录内容与填写一致
3. 质量新增一条实例，状态为运行中
4. 1）质量实例：校验未通过，查看明细展示doris1中的三条数据
2）离线实例：运行成功，状态更新为【成功】，运行/执行结果符合预期，日志无报错
5. 1）补数据质量工作流中有1个规则包，任务都运行成功，状态更新为【成功】，运行/执行结果符合预期，日志无报错
2）质量生成一条实例，校验未通过

---

## 验证【联动】验证质量和离线绑定，单规则包-【表行数-校验通过】结果正确
**优先级**: P2
**前置条件**: 1、doris数据源已存在表
建表语句：
drop table if exists doris1;
...
3. 等待离线实例运行，查看质量实例
4. 运行结束后，查看离线和质量实例的运行结果
5. 进入离线，对doris2doris任务补数据-当前任务及下游，查看结果
...
**步骤**:
1. 进入【数据资产-数据质量】，新建单表校验，选择表doris2
2. 新建多表比对，选择前置中的表doris1、doris2,过滤条件都为part_date='${utc0yyyy_MM_dd_1d}'
3. 同名映射，勾选id为主键
4. 勾选【记录数百分比差异】-34%
5. 创建规则
6. 立即运行规则，查看结果
7. 编辑规则，修改【记录数百分比差异】-50%
8. 立即运行规则，查看结果

**预期**:
1. 成功进入【数据资产-数据质量】，新建单表校验，选择表doris2页面，页面内容正常加载显示，无报错
2. 进入选择字段页面
3. 勾选成功，系统给出成功反馈，相关页面/数据状态更新为最新
4. 勾选成功，系统给出成功反馈，相关页面/数据状态更新为最新
5. 系统提示创建成功，列表中出现新创建的记录，记录内容与填写一致
6. 质量实例校验不通过
...
**步骤**:
1. 进入【数据资产-数据质量】，新建单表校验，选择表doris2
2. 新建多表比对，左表选择前置中的表doris_demo_data_types_source，点击数据预览查看结果
3. 右表选择前置中的表doris_demo_data_types_source，点击数据预览查看结果

**预期**:
1. 成功进入【数据资产-数据质量】，新建单表校验，选择表doris2页面，页面内容正常加载显示，无报错
2. 数据预览展示三条数据，数据正确，内容与预期完全一致，无异常或错误
3. 数据预览展示三条数据，数据正确，内容与预期完全一致，无异常或错误
...
**步骤**:
1. 进入【数据资产-数据质量】，新建单表校验，选择表doris2
2. 新建多表比对，左表选择前置中的表doris_demo_data_types_source,过滤条件为part_date='${utc0yyyy_MM_dd_1d}'
...
4. 勾选成功，系统给出成功反馈，相关页面/数据状态更新为最新
5. 勾选成功，系统给出成功反馈，相关页面/数据状态更新为最新
6. 系统提示创建成功，列表中出现新创建的记录，记录内容与填写一致
7. 质量实例校验不通过
8. 系统提示修改成功，页面显示修改后的最新内容
9. 质量实例校验通过
...
## 验证【多表-同源比对】功能正常
**优先级**: P2
**前置条件**: doris数据源已存在表doris_demo_data_types_source、doris_demo1_data_types_source
...
**步骤**:
1. 进入【数据资产-数据质量】，新建单表校验，选择表doris2
2. 新建多表比对，左表选择前置中的表doris_demo_data_types_source
...
2）不一致的不映射并提示：主表与对照表字段类型不一致，无法比对
5. 勾选成功，系统给出成功反馈，相关页面/数据状态更新为最新
6. 勾选成功，系统给出成功反馈，相关页面/数据状态更新为最新
7. 系统提示创建成功，列表中出现新创建的记录，记录内容与填写一致
8. 质量实例校验不通过
...
**步骤**:
1. 进入【数据资产-数据质量】，新建单表校验，选择表doris2
2. 新建规则集，选择前置的doris表，点击下一步
3. 下载规则模板，填写前置中的规则sql，上传
4. 配置周期调度，点击创建
5. 立即执行，查看运行结果
...
1. 成功进入【数据资产-数据质量】，新建单表校验，选择表doris2页面，页面内容正常加载显示，无报错
2. 进入规则配置
3. 上传成功,规则导入正确，内容与预期完全一致，无异常或错误
4. 系统提示创建成功，列表中出现新创建的记录，记录内容与填写一致
5. 运行结果正确，内容与预期完全一致，无异常或错误
...
**步骤**:
1. 进入【数据资产-数据质量】，新建单表校验，选择表doris2
2. 新建单表校验，选择前置中的doris表，点击数据预览，查看结果
...
**优先级**: P2
**前置条件**: DROP TABLE IF EXISTS doris_test;
...
3. 选择and-空值数-固定值>1,保存后立即运行
4. 修改规则and-空值数-固定值>=1,保存后立即运行
5. 修改规则or-空值数-固定值>1,保存后立即运行

**预期**:
1. 成功进入【数据资产-数据质量】，新建单表校验，选择表doris2页面，页面内容正常加载显示，无报错
2. 新增【字段间规则逻辑】下拉框，值为and/or
3. 运行不通过
4. 运行通过
5. 运行通过
...
1. 成功进入【数据资产-数据质量】，新建单表校验，选择表doris2页面，页面内容正常加载显示，无报错
2. 系统提示新建成功，列表顶部出现新建的记录，记录内容与填写一致
3. 结果正确，内容与预期完全一致，无异常或错误
4. 系统提示编辑成功，修改内容已更新，页面显示最新内容
5. 结果正确，内容与预期完全一致，无异常或错误

---

## 验证【联动】验证质量和离线绑定，多规则包-【表行数-强规则-校验不通过】-校验结果正确
**优先级**: P1
**前置条件**: 1、doris数据源已存在表
建表语句： 
...
调度：自动关联离线调度周期。规则包个数3
绑定任务：前置条件saprk2spark
2. 等待离线周期实例运行，进入【数据资产-数据质量-任务实例查询】
3. 进入【离线开发-运维中心-周期任务实例】
4. 进入【离线开发-运维中心-周期任务】，对saprk2spark任务补当前数据及下游，查看结果

**预期**:
1. 系统提示新建成功，列表顶部出现新建的记录，记录内容与填写一致
2. 1）页面生成一条实例，校验未通过
2）查看明细展示doris_demo_data_types_source中的三条数据
3. 离线任务运行失败
4. 1）补数据质量工作流中有3个规则包，离线任务运行失败，报错质量任务校验不通过
2）质量生成一条实例，校验不通过

---

## 验证【联动】验证质量和离线绑定，单规则包-【表行数-添加过滤条件】-校验结果正确
**优先级**: P1
**前置条件**: 1.spark数据源已存在表
建表语句：
...
调度：自动关联离线调度周期。规则包个数1
绑定任务：前置条件spark2spark
3. 等待离线实例运行，查看质量实例
4. 运行结束后，查看离线和质量实例的运行结果
5. 进入离线，对spark2spark任务补数据-当前任务及下游，查看结果
...
## 验证【联动】验证质量和离线绑定，单规则包-【表行数-强规则-校验不通过】结果正确
**优先级**: P1
**前置条件**: 1、saprk数据源已存在表
建表语句：
...
3. 等待离线实例运行，查看质量实例
4. 运行结束后，查看离线和质量实例的运行结果
5. 进入【离线开发-运维中心-周期任务】，对spark_2_spark任务补当前数据及下游，查看结果
...
5. 1）补数据质量工作流中有1个规则包，离线任务运行失败，报错质量任务校验不通过
2）质量生成一条实例，校验不通过
...
2. 系统提示新建成功，列表顶部出现新建的记录，记录内容与填写一致
3. 生成一条实例，运行中
4. 运行结果正确，内容与预期完全一致，无异常或错误
5. 周期时间生成一条实例，运行正确，内容与预期完全一致，无异常或错误
...
**步骤**:
1. 进入【数据资产-数据质量】，新建单表校验，选择表doris2
2. 新建单表规则，选择前置中的doris表
规则选择自定义sql，输入sql使校验通过，如：[⚠️F04 请替换为具体测试数据]select * from doris;固定值<10000
3. 立即运行查看结果
4. 修改规则，配置使其校验不通过
5. 立即运行查看结果
...
【管理员】必填项未填提示；
3. 质量项目创建成功，系统给出创建成功提示，列表中出现新创建的记录
4. 项目置顶成功，提示“操作成功”
5. 项目删除成功，系统给出删除成功提示，该记录从列表中消失
