...
`,
  ...
  assertIncludes(fixedArchiveFrontMatter, "suite_name: 【数据质量】质量校验不通过时支持发送邮件", "archive 写入 canonical suite_name");
...
`,
  ...
  console.log("\n=== Test: dtstack prd_id=0 is preserved and not treated as missing ===");
  const zeroPrdArchiveFile = resolve(
    tempRoot,
    "cases/archive/data-assets/v6.4.9/零号需求校验.md",
  );
  writeFile(
    zeroPrdArchiveFile,
    `---
...
`,
  ...
  const zeroPrdReport = runNodeScript(cliPath, [
    "--root",
    tempRoot,
    "--archive",
    "--path",
  ...
  assert(zeroPrdReport.code === 0, "prd_id=0 不会被误报为缺失", [
    zeroPrdReport.stdout.trim(),
    zeroPrdReport.stderr.trim(),
  ].filter(Boolean));
  assert(!zeroPrdReport.stdout.includes("missing prd_id"), "prd_id=0 审计报告不包含 missing prd_id");

  const zeroPrdFix = runNodeScript(cliPath, [
    "--root",
    tempRoot,
    "--archive",
    "--path",
  ...
  assert(zeroPrdFix.code === 0, "prd_id=0 fix 成功", [
    zeroPrdFix.stdout.trim(),
    zeroPrdFix.stderr.trim(),
  ].filter(Boolean));
  assertIncludes(readFileSync(zeroPrdArchiveFile, "utf8"), "prd_id: 0", "prd_id=0 在 fix 后被保留");
