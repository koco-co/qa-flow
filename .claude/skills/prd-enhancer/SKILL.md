...
| 4 | 逐张读取图片 | 产出多模态识别结果 |
| 5 | 生成页面要点 | 按 `references/page-insight-format.md` 写入 `图N 页面要点`；**图片中无法直接读出的内容不得填写，记入识别限制** |
...
| 9 | **汇总不确定项并向用户确认** | 收集所有无法从原文直接读出的内容，以带选项格式统一提问，**等待用户回复后更新文档** |
| 10 | 向用户展示增强摘要 | 汇总已确认内容、仍待确认项、健康度与下一步建议 |
...
3. 每张已处理图片下方都写入 `图N 页面要点`，**图片中无法直接读出的内容已记入识别限制，未擅自填写推测内容**。
4. 输出文件符合 `references/prd-template.md` 约定，front-matter 状态写回为中文。
5. 图片引用已标准化，增强文件路径已在终端输出。
6. **所有不确定项已以带选项格式向用户提问，并等待用户回复后更新文档**。
7. 已输出 PRD 健康度预检结果与增强摘要。
8. **⛔ 在用户明确说「可以开始写用例」或「需求已确认」之前，不得触发测试用例生成**（无论由用户直接触发还是 test-case-generator 自动调用）。
