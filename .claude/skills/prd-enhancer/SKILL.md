...
3. 以 **A/B/C 选项卡** 格式展示问题，AI 推荐选项标注 `← 推荐`：
...
【Q2】验收标准 ★★  {操作}成功后，页面如何反馈用户？

  A  弹出 Toast 提示「{操作}成功」，列表自动刷新  ← 推荐（常见模式）
  B  跳转回列表页，新记录显示在首行
  C  弹窗关闭，列表自动刷新
  D  跳过，AI 自动推断
  E  自定义输入...
...
4. 收集用户回答（1 轮，字母选择或自定义输入均可）：
   - 选择字母 → 记录对应选项内容，标记 `confirmed`
   - 选末位字母（自定义）→ 等用户输入后记录，标记 `user_custom`
   - 结果追加到 PRD 末尾的 `## 需求澄清结果` 章节
...
| 用户回复 | 行为 |
|---------|------|
| 字母选项（如 `Q1: A  Q2: B`） | 记录对应内容，继续 Step 0.5 |
| 「全部推荐」 | 接受所有 A 选项，继续 Step 0.5 |
| 「直接开始」/ 「不用澄清」 | 跳过，记录评分，继续 Step 0.5 |
| 部分回复 | 已选项记录，未覆盖项标记 `inferred`，继续 |
