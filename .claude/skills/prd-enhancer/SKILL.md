...
1. 读取 PRD 文件，按 Tier 1 三个维度快速评估可测试性（参考 `test-case-generator/references/elicitation-dimensions.md`）：
   ...
   - `field_linkage`：存在多字段表单但未描述联动规则 → 询问是否有字段间依赖关系
   - `state_machine`：需求提及流程/审批/状态，但无状态流转图 → 询问状态和流转条件
...
- `## 三、页面元素清单` 下的 `### 3.x` 对应操作按钮、表单字段、枚举选项等元素分组
- `## 四、字段联动规则` 统一描述字段间联动/显隐/级联逻辑
- `## 五、业务规则与校验逻辑` 包含提交校验规则（带规则编号 V-xxx）和业务逻辑规则
- `## 六、流程与状态机` 描述审批节点和状态流转图
...
- `field_linkage_missing`：表单字段 ≥ 3 个但无四、字段联动规则章节 → WARNING W005
- `validation_rules_missing`：存在表单提交功能但无五、业务规则与校验逻辑章节 → WARNING W006
- `exclusion_missing`：功能模块 ≥ 2 个但无八、本期不涉及的功能章节 → INFO I001
