...
1. 根据报错信息中的 Java 包名、Service 类、Controller 名，从 `config.repos` 推断最可能的仓库和分支
...
## 派发 backend-bug-agent

派发 `backend-bug-agent`（model: sonnet），输入：

- 完整报错日志（含 Java 异常堆栈、`Caused by` 链、SQL 异常等）
- 源码上下文（可选，若 E1 同步成功）
- 环境信息（JDK 版本、Spring Boot 版本等，缺失时标注"未提供"）

Agent 返回 `backend_bug_json`：
