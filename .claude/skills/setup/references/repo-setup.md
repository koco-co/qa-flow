...
> 本文件为 setup skill 步骤 3 的详细参考。源码仓库仅供只读分析，严禁 push 或 commit。
...
```bash
git -C workspace/.repos/{group}/{repo} fetch origin
git -C workspace/.repos/{group}/{repo} checkout <branch>
git -C workspace/.repos/{group}/{repo} pull origin <branch>
```
