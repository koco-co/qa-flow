# ui-autotest · step 5 — 执行测试（全量回归）

> 由 SKILL.md 路由后加载。执行时机：步骤 4（合并）完成后。

按 Task Schema 更新：将 `步骤 5` 标记为 `in_progress`。

> 此步骤为合并后的全量回归验证，因步骤 3 已逐条验证通过，此处预期全部通过。
...
按 Task Schema 更新：将 `步骤 5` 标记为 `completed`（subject: `步骤 5 — 回归完成，{{passed}}/{{total}} 通过`）。
