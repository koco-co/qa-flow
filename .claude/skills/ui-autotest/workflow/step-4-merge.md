# ui-autotest · step 4 — 合并脚本

> 由 SKILL.md 路由后加载。执行时机：步骤 3（Subagent A）完成后。

按 Task Schema 更新：将 `步骤 4` 标记为 `in_progress`。

> 仅合并步骤 3 中**验证通过**的脚本。

```bash
bun run .claude/skills/ui-autotest/scripts/merge-specs.ts \
  --input .kata/{{project}}/ui-blocks/{{suite_slug}}/ \
...
按 Task Schema 更新：将 `步骤 4` 标记为 `completed`（subject: `步骤 4 — 合并 {{n}} 条脚本`）。
