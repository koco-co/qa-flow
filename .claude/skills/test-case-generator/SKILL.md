...
6. **Writer 步骤**：
   ...
   - **6d. BLOCKED 中转协议**（Writer 或 Reviewer 返回 `## BLOCKED` 时触发）：
     1. 将该 Writer 状态写为 `blocked`，将问题列表写入 `writers.<name>.blocked_questions`
     2. **编排器（主 Agent）直接将问题展示给用户**：
        ```
        ⚠️ Writer「<模块名>」需要你确认以下信息：
        ...
        请回答后，我将继续生成该模块的用例。
        ```
     3. 等待用户回复，将答案写入 `writers.<name>.confirmed_answers`
     4. 重新启动该 Writer Agent，在 prompt 末尾追加：
        ```
        ...
        2. <问题2> → <用户答案2>
        ```
     5. 若重新启动后仍返回 `## BLOCKED`（新问题），重复上述流程；同一问题不得重复追问（答案已给出则必须直接使用）
