...
>
> 📌 **用户侧推荐写法**：快速模式优先写成 `为 Story-20260322 --quick 生成测试用例`。自然语言“快速生成测试用例”也会被识别为同一模式，但文档与验收提示统一以 `--quick` 表达。
...
跳过 Step 3（Brainstorming）、Step 4（Checklist 预览）、Step 5（用户确认），自动执行其余流程。适合重跑、已熟悉的需求、或无暇交互的场景。

```
为 Story-20260322 --quick 生成测试用例
根据需求文档: Story-20260322 中的 PRD-26 --quick 生成测试用例
```

> 自然语言“快速生成测试用例”会被识别为相同模式，但推荐文档与提示统一使用 `--quick`。
...
输入中的「模块」默认指现有输出中的页面/模块标签（通常对应 L2/L3，如 `列表页`、`详情页`、`搜索功能`），不是 `.claude/config.json` 里的模块 key。

### 续传 vs 模块重跑 vs 全量重跑

| 场景 | 触发方式 | 会保留什么 | 不会做什么 |
| --- | --- | --- | --- |
| 续传 | `继续 Story-20260322 的用例生成` 或重发原命令 | 复用 `.qa-state.json`、已完成 Writer 结果、已生成产物；从中断步骤或验收阶段继续 | 不删除已完成结果；不会自动把 `failed` Writer 当成已重试 |
| 模块级重跑 | `重新生成 Story-20260322 的「列表页」模块用例` | 保留其他模块结果，只重跑目标 Writer，然后重新 Reviewer + XMind | 不重新增强未变更的 PRD，不重跑其他模块 |
| 全量重跑 | 删除 `.qa-state.json` 和相关 `*-enhanced.md` 后，重新发起完整流程或快速模式指令 | 从 Step 1 全新开始，不复用旧状态或旧增强产物 | 这不是“继续”，也不是“模块重跑” |
...
Step 3: Brainstorming + 解耦分析（快速模式时跳过）
Step 4: Checklist 预览（快速模式时跳过）
Step 5: 用户一次确认（快速模式时跳过）
...
> **快速模式跳过此步，直接进入 Step 4（同样跳过）→ Step 6。**
...
断点续传时，如果读取到 `awaiting_verification: true`，只重新展示本节提示，不重新执行 9.1。`确认通过` / `已修改，请同步` 这两个固定回复仅用于测试用例生成流程，不用于单独 PRD 增强、Bug 分析或冲突分析。
