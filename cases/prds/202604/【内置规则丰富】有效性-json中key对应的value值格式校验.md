...
- 评审补充：需覆盖千级以上、多层级 key/value 配置场景；需验证 key 配置关联质量任务后即使被通用配置删除，历史规则回显与任务执行仍正常；质量规则需补充抽样、分区与脏数据存储回归场景。
...
- 当 key 规模达到千级以上且存在多层级时，仍需支持搜索、勾选、预览与运行匹配。
...
- 已关联质量任务的 key/value 配置即使后续在 `json格式校验管理` 页面被删除，历史规则回显与质量任务运行仍应保持正常。
- 质量任务执行需覆盖抽样、分区与脏数据存储目标库变更后的回归场景。
...
6. 评审补充口径：历史已绑定任务与结果不因上游 key/value 配置删除而失效，抽样/分区/脏数据存储配置变更后仍需保持执行结果可用。
...
3. 校验失败时可查看明细与日志。
4. 抽样、分区场景下保存并运行 `格式-json格式校验` 结果正确。
...
5. 千级以上多层级 key/value 场景下，需通过脚本生成 xlsx 并导入通用配置后完成搜索、勾选、预览与回显验证。
6. key/value 配置关联质量任务后，即使在通用配置中删除，历史规则回显与质量任务执行仍正常。
7. 脏数据存储目标库变更后，失败数据落库功能保持正常。
