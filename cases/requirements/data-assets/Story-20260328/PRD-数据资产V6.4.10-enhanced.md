...
- Source: https://lanhuapp.com/web/#/item/project/product?tid=24a1c6b2-a52e-454c-8d51-8aff866598b1&pid=7de90493-e80f-4592-a263-38fb2d2e98c0&versionId=3d217b4a-1614-4fd6-8f1e-ac6a24497ee0&docId=fc0fee93-74f5-4eff-a769-99e68506b296&docType=axure&pageId=5592c6c6018548609321a0a0df65744d&image_id=fc0fee93-74f5-4eff-a769-99e68506b296&parentId=7f94f9b4-05f7-436a-abf3-3bcc1d09d99a
- Extracted via: lanhu-mcp text_only tester mode
- Project path: 团队文件/数据资产
- Total pages: 12
- Generated at: 2026-03-28T07:15:50.098028Z

> Enhanced note: This validation run uses Lanhu text extraction as the upstream PRD source. No Obsidian image embeds were present, so the enhanced file preserves the extracted content and metadata for downstream case generation.


⚡ 📝 TEXT_ONLY MODE | Version: 3d217b4a...
📊 Total 12/12 pages

============================================================
📝 STAGE 1: GLOBAL TEXT SCAN (Building God's View)
============================================================
🎯 Your Mission:
  1. Quickly read ALL page texts below
  2. Identify document structure (modules, flows, entities)
  3. Output structured analysis (MUST use Markdown table)
  4. Design grouping strategy based on business logic
  5. Create TODOs for STAGE 2 detailed analysis

⚠️ Important:
  • This is text-only mode for fast overview
  • No visual outputs in this stage
  • Focus on understanding structure, not extracting details
  • Details will be extracted in STAGE 2 (with images)
============================================================
...
1️⃣ 快速阅读所有页面文本
2️⃣ 输出文档结构表（模块、页面、功能）
3️⃣ 识别业务关联关系
4️⃣ 设计合理分组策略（基于业务逻辑）
5️⃣ ⚡【必须】询问用户选择分析模式
6️⃣ 反向更新TODOs（细化STAGE2分组任务）

============================================================
⚠️ 【重要】完成扫描后必须询问用户选择分析模式：
============================================================
全部页面已浏览完毕。
...
============================================================
📝 ALL PAGE TEXTS (For Global Understanding)
============================================================
💡 Read these texts to understand document structure
💡 Identify modules, flows, and business logic
💡 Then design reasonable grouping strategy for STAGE 2
============================================================



────────────────────────────────────────────────────────────
📄 Page 1: 15525【内置规则丰富】一致性，多表数据一致性比对
────────────────────────────────────────────────────────────
...
对比表为xx;xx;xx(表名)，逻辑主键未匹配的共xx条数据，逻辑主键匹配，但数据不匹配的共xx条数据
...
────────────────────────────────────────────────────────────
📄 Page 2: 15529【内置规则丰富】合理性，单表，字段值的计算关系对比
────────────────────────────────────────────────────────────
[Flowchart/Component Text]
...
────────────────────────────────────────────────────────────
📄 Page 3: 15530【内置规则丰富】合理性，多表，字段大小对比以及字段计算逻辑对比
────────────────────────────────────────────────────────────
[Flowchart/Component Text]
...
────────────────────────────────────────────────────────────
📄 Page 4: 15693【内置规则丰富】完整性，json中key值范围校验
────────────────────────────────────────────────────────────
[Flowchart/Component Text]
...
────────────────────────────────────────────────────────────
📄 Page 5: 15694【内置规则丰富】有效性，json中key对应的value值格式校验
────────────────────────────────────────────────────────────
[Flowchart/Component Text]
...
────────────────────────────────────────────────────────────
📄 Page 6: 15695【内置规则丰富】有效性，支持设置字段多规则的且或关系
────────────────────────────────────────────────────────────
[Flowchart/Component Text]
开发版本：6.3岚图定制化分支需求内容：有效性校验支持校验取值范围和枚举值的合并判断 | 规则配置 | 结果查询 | 针对“校验不通过”的规则，支持查看明细，明细记录不符合规则的数值 | 查看“有效性校验”明细 | 记录不符合要求的数据，数据列表保留全部字段，校验字段标红展示，下载明细数据中校验字段也标红展示 | 标题文案修改为： | 针对“校验通过”的规则，不记录明细数据；针对“校验失败”的规则，支持查看日志 | 规则类型 | 字段级 | 字段 | xxx | 取值范围 | 期望值>1且<10 | 枚举值 | 包含/不包含1.2.3 | 取值范围和枚举值关系 | 且 | 过滤条件 | 强弱规则
...
第二行：枚举值设置，逻辑和枚举值规则保持一致，支持选择包含/不包含
...
────────────────────────────────────────────────────────────
📄 Page 11: 15699【数据资产】落标检查任务、元数据同步任务支持选择资源组和配置环境参数
────────────────────────────────────────────────────────────
[Flowchart/Component Text]
