...
| 2    | 找到「json格式校验任务\_P0」，点击操作列的【编辑规则】按钮，进入规则配置页面；在有效性校验区域点击【添加规则】，按如下配置填写：<br>- 字段：info（json）<br>- 统计规则：格式-json格式校验<br>- 校验key：勾选「person-name」和「person-age」<br>点击【保存】按钮 | 规则保存成功：<br>1) 规则列表中显示新增规则行<br>2) 「统计规则」列显示「格式-json格式校验」<br>3) 「校验key」列回显「person-name;person-age」                                                                                                                                                |
| 3    | 返回规则任务列表，找到「json格式校验任务\_P0」，点击操作列的【立即执行】按钮，等待任务状态变为「已完成」（最长等待30秒）                                                                                                                                        | 任务执行完成：<br>1) 状态列显示「已完成」<br>2) 最新校验结果列显示「校验通过」                                                                                                                                                                                                               |
| 4    | 点击「json格式校验任务\_P0」行的【查看报告】入口，进入质量报告页面，找到「格式-json格式校验」规则对应行，查看各列内容                                                                                                                                           | 质量报告中该规则行展示：<br>1) 规则类型列=「有效性校验」<br>2) 规则名称列=「格式-json格式校验」<br>3) 字段类型列=「json」<br>4) 质检结果列=「校验通过」<br>5) 未通过原因列=「--」<br>6) 详情说明列=「符合规则key为"person-name;person-age"时的value格式要求」<br>7) 操作列无「查看详情」链接 |
