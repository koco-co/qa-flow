...
for (const datasource of ACTIVE_DATASOURCES) {
  test.describe(`${"【内置规则丰富】完整性，json中key值范围校验 - 规则集管理"} - ${datasource.reportName}`, () => {
    ...
    test("验证校验内容下拉框支持输入关键词搜索查询", async ({ page }) => {
      ...
      const key1Dropdown = await searchVerificationContent(page, ruleForm, "key1");
      await expect(key1Dropdown).toContainText("key1");
      await expect(key1Dropdown).toContainText("key11");

      const provinceDropdown = await searchVerificationContent(page, ruleForm, "省份");
      await expect(provinceDropdown).toContainText("key11");

      const noneDropdown = await searchVerificationContent(page, ruleForm, "xyz_not_exist");
    ...
    test("验证校验内容回显格式与悬浮展示正确", async ({ page }) => {
      ...
      await setVerificationContent(page, ruleForm, ["key1", "key2", "key11", "key22"]);
    ...
    test("验证key数据量可展示现有维护项", async ({ page }) => {
      ...
      expect(options).toEqual(expect.arrayContaining(["全部", "key1", "key2", "key11", "key22"]));
    });
  });
}
