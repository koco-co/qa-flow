    ...
    try {
      ...
      await step(
        ...
        async () => {
          ...
          // 验证列头（key、中文名称、数据源类型）可见
          const thead = page.locator(".ant-table-thead");
          await expect(thead.filter({ hasText: "key" })).toBeVisible({ timeout: 5000 });
          await expect(thead.filter({ hasText: "中文名称" })).toBeVisible({ timeout: 5000 });
          await expect(thead.filter({ hasText: "数据源类型" })).toBeVisible({ timeout: 5000 });
          await expect(thead.filter({ hasText: "创建人" })).toBeVisible({ timeout: 5000 });
          await expect(thead.filter({ hasText: "操作" })).toBeVisible({ timeout: 5000 });
        },
        table,
      );
      ...
      await step(
        ...
        async () => {
          ...
          // value格式 为空时，测试数据区域不应存在
          const regexArea = modal.locator(
            "[class*='regexTest'], [class*='regex-test'], [class*='RegexTest']",
          );
          const regexAreaExists = await regexArea.isVisible({ timeout: 1000 }).catch(() => false);
          // 如果区域已存在则无需断言（可能默认展示），否则验证其不可见
          if (!regexAreaExists) {
            await expect(regexArea).not.toBeVisible({ timeout: 1000 }).catch(() => undefined);
          }
        },
      ...
      await step(
        ...
        async () => {
          ...
          // 等待匹配结果出现：实际文本为「符合正则」（TestResult.Pass 对应 CF key）
          await expect(
            modal.locator("*").filter({ hasText: /符合正则/ }).last(),
