...
test.describe("【通用配置】json格式配置 - 通用配置-json格式校验管理", () => {
  test("【P1】验证批量删除多条key（含子层级）", async ({ page, step }) => {
    // 使用共同前缀 batchDel，以便后续用单次搜索同时定位两条记录
    ...
    await step("前置步骤: 新增 batchKey1（含子层级 batchKey1Child）和 batchKey2 → 前置数据准备完成", async () => {
    ...
    // 步骤1：刷新页面，搜索 sharedPrefix 确认两条记录正常加载
    ...
    // 步骤2：勾选 batchKey1 和 batchKey2 所在行的复选框，断言批量操作栏出现
