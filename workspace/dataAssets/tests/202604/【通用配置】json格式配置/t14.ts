...
test.describe("【通用配置】json格式配置 - 通用配置-json格式校验管理", () => {
  test("【P1】验证5层层级展开下钻及展开图标显示逻辑", async ({ page, step }) => {
    ...
    await step("前置: 创建5层嵌套结构及独立叶子节点", async () => {
    ...
    await step(
      ...
      async () => {
        ...
        const rootRow = page.locator(".ant-table-row").filter({ hasText: rootKey }).first();
        ...
        await searchKey(page, leafKey);
        const leafRow = page.locator(".ant-table-row").filter({ hasText: leafKey }).first();
