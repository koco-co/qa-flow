...
test.describe("【通用配置】json格式配置 - 通用配置-json格式校验管理", () => {
  test("【P1】验证value格式有内容时正则测试控件显示及匹配通过失败场景", async ({ page, step }) => {
    ...
    await step(
      ...
      async () => {
        ...
        const testDataInput = modal
          ...
          .first();
        const regexTestBtn = modal.getByRole("button", {
          name: /正则匹配测试|正则测试|匹配测试/,
        });
    ...
    const matchResult = modal.locator("*").filter({ hasText: /匹配成功|符合正则/ }).last();
    await step(
      ...
      async () => {
        ...
        await expect(
          modal.locator("*").filter({ hasText: /匹配成功|符合正则/ }).last(),
    ...
    const failResult = modal.locator("*").filter({ hasText: /匹配失败|不符合正则/ }).last();
    await step(
      ...
      async () => {
        ...
        await regexTestBtn.click();
        await expect(
          modal.locator("*").filter({ hasText: /匹配失败|不符合正则/ }).last(),
        ).toBeVisible({ timeout: 10000 });
      },
      failResult,
    );
