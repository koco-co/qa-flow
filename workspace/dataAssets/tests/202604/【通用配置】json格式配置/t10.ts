...
test.describe("【通用配置】json格式配置 - 通用配置-json格式校验管理", () => {
  test("【P1】验证单个删除含子层级的key会联动删除子层级数据", async ({ page, step }) => {
    ...
    // 前置：创建父 key 及其子层级
    await step("前置步骤: 新增父 key 及子层级 → 前置数据准备完成", async () => {
    ...
    await step(
      ...
      async () => {
        ...
        // 搜索父 key，因为表格可能有多页数据导致目标行不在首页
        // 直接填充搜索框并点击搜索按钮，确保 onSearch 被触发
        const searchInput = page.locator(".dt-search input").first();
        await searchInput.waitFor({ state: "visible", timeout: 10000 });
        await searchInput.clear();
        await searchInput.fill(deleteParent);
        // 点击搜索按钮（比 Enter 更可靠地触发 onSearch）
        await page.locator(".dt-search .ant-input-search-button").first().click();
        // 等待 spin 出现再消失（fetchTree 调用完成）
        await page
