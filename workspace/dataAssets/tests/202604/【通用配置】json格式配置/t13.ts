    ...
    try {
      ...
      await step("步骤2: 新增 SparkThrift2.x 测试key → 记录创建成功", async () => {
        ...
        for (let attempt = 1; attempt <= 2; attempt++) {
          ...
          await gotoJsonConfigPage(page);
          await searchKey(page, sparkKey);
          const row = page.locator(".ant-table-row").filter({ hasText: sparkKey }).first();
      ...
      await step("步骤3: 筛选数据源类型=SparkThrift2.x → 列表可命中 Spark 测试数据", async () => {
        ...
        await searchKey(page, sparkKey);
        await expect(
          page.locator(".ant-table-row").filter({ hasText: sparkKey }).first(),
