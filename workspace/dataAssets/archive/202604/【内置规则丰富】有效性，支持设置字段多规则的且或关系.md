...
| 2 | 在规则任务列表中点击任务 task_15695_and 对应行的【表名】展开抽屉弹窗，在抽屉中点击【立即执行】按钮，等待任务执行完成（最长等待 120 秒），进入【数据质量 → 校验结果查询】页面查看最新记录 | 校验结果查询列表中出现该任务的最新执行记录，由于数据中存在不符合规则项（id=2、4、5），整条记录的状态列显示「校验异常」 |
| 3 | 在校验结果查询列表中，点击该任务记录对应的【表名】展开详情抽屉，抽屉顶部显示任务名（如 `task_15695_and_doris3_x`），在【监控报告】Tab 下查看规则未通过统计按钮和有效性校验卡片内容 | 监控报告 Tab 下显示如下：<br>1) 顶部显示统计按钮「校验未通过(1)」（表示本次有 1 条规则校验未通过）<br>2) 有效性校验卡片中：字段列显示「score」<br>3) 统计函数列显示「取值范围&枚举范围」<br>4) 取值范围列显示「期望值 >1 且 <10」<br>5) 枚举值列显示「in 1,2,3」，取值范围和枚举值关系列显示「且」<br>6) 强弱规则列显示「强规则」，卡片右上角显示【查看明细】与【查看趋势】操作链接 |
| 4 | 切换至 doris3.x 数据源，按相同表、规则包和配置重复以上步骤。 | 切换至 doris3.x 数据源重复以上步骤后，规则任务创建、执行、结果展示与 sparkthrift2.x 场景一致。 |

<!-- 由 ui-autotest 自测校正（2026-04-20）：
     - step 2/3 流程按实际产品调整：任务执行入口是点击表名展开抽屉后「立即执行」；结果查看入口是「校验结果查询」而非「质量报告」
     - 外层状态「校验异常」包含所有未通过/运行失败情况；抽屉内通过「校验未通过(N)」按钮指示具体规则未通过数量
     - 实际 UI 操作链接文案为「查看明细」「查看趋势」，非原描述的「查看详情」
     - step 3 的 6 条子断言按抽屉实际显示的卡片列重写，去除「不符合有效性规则」「不符合规则...」等实际 UI 未显式呈现的文案 -->
